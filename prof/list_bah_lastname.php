...
function lastname_len($s)
{
    if (function_exists('mb_strlen')) return mb_strlen($s, 'UTF-8');
    return strlen($s);
}

function lastname_like($v)
{
    $v = str_replace(array('\\', '%', '_'), array('\\\\', '\\%', '\\_'), $v);
    return $v . '%';
}

function lastname_ops_svg($name)
{
    $d = array(
...
function lastname_pic($username)
{
...
function lastname_filter_hiddens($last_name, $name1, $fname, $aria)
{
    $pairs = array(
        'action' => '1',
        'last_name' => $last_name,
...
if ($name1 === '' && isset($_POST['name'])) $name1 = trim($_POST['name'] . '');
$last_name = isset($_POST['last_name']) ? trim($_POST['last_name'] . '') : '';
$fname = isset($_POST['fname']) ? trim($_POST['fname'] . '') : '';
$aria = isset($_POST['aria']) ? trim($_POST['aria'] . '') : '1';
if (!in_array($aria, array('1', '2', '3', '4', '5'), true)) $aria = '1';
...
if ($did_search) {
    if (lastname_len($last_name) < 3) {
        $field_errors['last_name'] = 'نام خانوادگی باید حداقل سه نویسه باشد';
    }
}

$page_title = (isset($title) && $title !== '') ? $title : 'جستجو براساس نام خانوادگی';
$pahneh_crumb = array(
    ...
    array('label' => 'جستجو براساس نام خانوادگی'),
);
?>
<!DOCTYPE html>
...
<body class="agri1-body agri1-page">
    ...
    <main class="agri1-main">
        <header class="agri1-hero">
            <h1 class="agri1-title" id="liste-title">جستجوی بهره‌بردار بر اساس نام و نام خانوادگی</h1>
        </header>
        <section class="agri1-card agri1-card-search" aria-labelledby="liste-title">
            <form id="reg-form" class="agri1-form" method="post" action="list_bah_lastname.php#1" novalidate>
                <h2 class="agri1-card-title">جستجو</h2>
                <div class="agri1-grid">
                    <div class="agri1-field" id="field-last_name">
                        <label class="agri1-label" for="last_name">نام خانوادگی شامل</label>
                        <input name="last_name" type="text" id="last_name" maxlength="50" autocomplete="off" value="<?php echo agri2_h($last_name); ?>" aria-invalid="<?php echo isset($field_errors['last_name']) ? 'true' : 'false'; ?>" aria-describedby="hint-last_name error-last_name"/>
                        <p class="agri1-hint" id="hint-last_name">حداقل سه نویسه — از ابتدای نام خانوادگی</p>
                        <p class="agri1-error<?php echo isset($field_errors['last_name']) ? '' : ' is-hidden'; ?>" id="error-last_name"><?php echo isset($field_errors['last_name']) ? agri2_h($field_errors['last_name']) : ''; ?></p>
                    </div>
                    <div class="agri1-field">
                        <label class="agri1-label" for="name1">نام شامل</label>
                        <input name="name1" type="text" id="name1" maxlength="50" autocomplete="off" value="<?php echo agri2_h($name1); ?>"/>
                    </div>
                    <div class="agri1-field">
                        <label class="agri1-label" for="fname">نام پدر شامل</label>
                        <input name="fname" type="text" id="fname" maxlength="50" autocomplete="off" value="<?php echo agri2_h($fname); ?>"/>
                    </div>
                    <div class="agri1-field">
                        <label class="agri1-label" for="aria">محدوده جستجو</label>
                        <select name="aria" id="aria" dir="rtl">
                            ...
                            <option value="4"<?php if ($aria == '4') echo ' selected="selected"'; ?>>مرکز جهاد کشاورزی</option>
...
if ($did_search && empty($field_errors)) {
    $where = 'name LIKE :name1 AND last_name LIKE :last_name AND fname LIKE :fname';
    $params = array(
        ':name1' => lastname_like($name1),
        ':last_name' => lastname_like($last_name),
        ':fname' => lastname_like($fname)
    );
    if ($aria === '2') {
        $where .= ' AND bah.id_ostan = :id_ostan';
        $params[':id_ostan'] = $id_ostan;
    } elseif ($aria === '3') {
        $where .= ' AND bah.id_ostan = :id_ostan AND bah.id_city = :id_city';
        $params[':id_ostan'] = $id_ostan;
        $params[':id_city'] = $id_city;
    } elseif ($aria === '4') {
        $where .= ' AND bah.id_ostan = :id_ostan AND bah.id_city = :id_city AND bah.id_mar = :id_mar';
        $params[':id_ostan'] = $id_ostan;
        $params[':id_city'] = $id_city;
        $params[':id_mar'] = $id_mar;
    } elseif ($aria === '5') {
        $where .= ' AND bah.mor_cod_m = :mor_cod_m';
        $params[':mor_cod_m'] = $login_session;
    }

    $query1 = "SELECT count(*) FROM bah WHERE $where";
    ...
    $query = "SELECT bah.add_abadi,bah.add_city,bah.no_bah,bah.date_s,bah.date_t,bah.tel_m,bah.name,bah.last_name,bah.fname,bah.co_name,bah.sh_meli,bah.bah_cod_m,bah.mor_cod_m
...
,list_city.ostan as ostan2,list_city.city as city2,list_city.shahr,list_city.mar
 ...
 where $where ORDER BY BINARY last_name ASC LIMIT " . (int) $start . ', ' . (int) $limit;
                ...
                <div class="agri1-col-picker" id="agri1-col-picker">
                    ...
                    <div id="agri1-col-panel" class="agri1-col-panel" role="group" aria-labelledby="agri1-col-panel-title">
                        ...
                        <ul class="agri1-col-list">
                            <li><label><input type="checkbox" data-col-toggle="ops" checked/> مشاهده</label></li>
                            ...
                            <li><label><input type="checkbox" data-col-toggle="date_t" checked/> تاریخ تولد</label></li>
                            <li><label><input type="checkbox" data-col-toggle="cod_m" checked/> کد ملی / شناسه ملی</label></li>
                            <li><label><input type="checkbox" data-col-toggle="fname" checked/> نام پدر</label></li>
            ...
            <div class="agri1-table-wrap">
                <table class="agri1-table">
                    <colgroup>
                        ...
                        <col data-col="expert" style="width:8%"/>
                        <col data-col="date_s" style="width:7%"/>
                        <col data-col="tel" style="width:8%"/>
                        <col data-col="date_t" style="width:7%"/>
                        <col data-col="cod_m" style="width:10%"/>
                        <col data-col="fname" style="width:7%"/>
                        ...
                        <col data-col="ostan" style="width:7%"/>
                        <col data-col="rownum" style="width:4%"/>
                    </colgroup>
                    <thead>
                        <tr>
                            <th class="agri1-ops-col" data-col="ops">مشاهده</th>
                            <th data-col="expert">کارشناس مروج</th>
                            <th data-col="date_s">تاریخ ثبت</th>
                            <th data-col="tel">شماره همراه</th>
                            <th data-col="date_t">تاریخ تولد</th>
                            <th data-col="cod_m">کد ملی / شناسه ملی</th>
                            <th data-col="fname">نام پدر</th>
...
<?php
        ...
        foreach ($stmt as $row) {
            ...
            $pic = lastname_pic($mor_cod_m1);
            $v_no_bah = ($row['no_bah'] == '1') ? 'حقیقی' : 'حقوقی';
            $expert_name = function_exists('user_name') ? str_replace('&nbsp;', ' ', user_name($mor_cod_m1)) : '';
?>
                        <tr>
                            <td class="agri1-ops" data-col="ops">
                                <div class="agri1-ops-bar">
                                    <form action="view_benef1.php#1" method="post" onsubmit="target_popup2(this)">
                                        ...
                                        <button type="submit" class="agri1-ops-btn" title="نمایش اطلاعات بهره‌بردار" aria-label="نمایش اطلاعات بهره‌بردار"><?php echo lastname_ops_svg('view'); ?></button>
                                    </form>
                                </div>
                            </td>
                            <td data-col="expert">
                                <div class="agri1-expert">
                                    ...
                                    <span dir="ltr"><?php echo agri2_h($mor_cod_m1); ?></span>
                                </div>
                            </td>
                            <td data-col="date_s"><?php echo agri2_h($row['date_s']); ?></td>
                            <td data-col="tel" dir="ltr"><?php echo agri2_h($row['tel_m']); ?></td>
                            <td data-col="date_t" dir="ltr"><?php echo agri2_h($row['date_t']); ?></td>
                            <td data-col="cod_m" dir="ltr"><?php echo agri2_h($row['bah_cod_m']); ?><br/><?php echo agri2_h($row['sh_meli']); ?></td>
                            <td class="agri1-name-col" data-col="fname"><?php echo agri2_h($row['fname']); ?></td>
                            ...
                            <td class="agri1-name-col" data-col="ostan"><?php echo agri2_h($row['ostan1']); ?><?php echo agri2_h($row['ostan2']); ?></td>
...
?>
        <nav class="agri1-pager" aria-label="صفحه‌بندی">
            <ul class="agri1-pager-list">
                <?php if ($id > 1) { ?>
                <li>
                    <form action="list_bah_lastname.php?id=<?php echo (int) ($id - 1); ?>#1" method="post">
                        <?php lastname_filter_hiddens($last_name, $name1, $fname, $aria); ?>
                ...
                <?php if ($show_first) { ?>
                <li>
                    <form action="list_bah_lastname.php?id=1#1" method="post">
                        <?php lastname_filter_hiddens($last_name, $name1, $fname, $aria); ?>
                ...
                <li>
                    ...
                    <?php } else { ?>
                    <form action="list_bah_lastname.php?id=<?php echo (int) $i; ?>#1" method="post">
                        <?php lastname_filter_hiddens($last_name, $name1, $fname, $aria); ?>
                ...
                <li><span class="agri1-pager-ellipsis">...</span></li>
                    <?php } ?>
                <li>
                    <form action="list_bah_lastname.php?id=<?php echo (int) $total; ?>#1" method="post">
                        <?php lastname_filter_hiddens($last_name, $name1, $fname, $aria); ?>
            ...
            <div class="agri1-pager-jump">
                <form id="pageJumpForm" action="list_bah_lastname.php" method="post">
                    <?php lastname_filter_hiddens($last_name, $name1, $fname, $aria); ?>
                    <span>به صفحه</span>
        ...
        (function () {
            ...
            form.addEventListener('submit', function (e) {
                ...
                if (!isNaN(pageId) && pageId >= 1 && pageId <= <?php echo (int) $total; ?>) {
                    this.action = 'list_bah_lastname.php?id=' + pageId + '#1';
                } else {
                    e.preventDefault();
                    input.focus();
                }
    ...
    <script>
        (function () {
            ...
            form.addEventListener('submit', function (e) {
                ...
                var v = last ? last.value.replace(/^\s+|\s+$/g, '') : '';
                var err = document.getElementById('error-last_name');
                var len = v.replace(/[\s\u200c]/g, '').length;
                if (len < 3) {
                    e.preventDefault();
                    if (last) last.setAttribute('aria-invalid', 'true');
                    if (err) { err.textContent = 'نام خانوادگی باید حداقل سه نویسه باشد'; err.classList.remove('is-hidden'); }
                    if (last) last.focus();
                    return;
                }
                busy = true;
        ...
        (function () {
            ...
            var KEY = 'list_bah_lastname_hidden_cols';
            function readHidden() {
                try {
