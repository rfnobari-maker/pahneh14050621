...
function agri_bvege_filter_hiddens($add_abadi, $add_city, $bah_cod_m, $z_sal, $b_time, $m_ab, $confi2, $cod_mah)
{
    $pairs = array(
        'action_lise' => '1',
        'add_abadi' => $add_abadi,
...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .agri1-ops form { display: inline-flex; margin: 0; }
        .agri1-ops button {
            display: inline-flex;
            align-items: center;
            justify-content: center;
        ...
        .agri1-ops button img {
            display: block;
            width: 28px;
            height: 22px;
        }
        .agri1-ops button:focus-visible {
            outline: 3px solid var(--color-ring);
            outline-offset: 2px;
        }
        .agri1-pager {
...
<body class="agri1-body agri1-page">
    ...
    <main class="agri1-main">
        <header class="agri1-hero">
            <h1 class="agri1-title" id="liste-b-vege-title">اطلاعات سطح برداشت و تولید قطعی بهره‌برداری‌های صیفی</h1>
        </header>

        <section class="agri1-card agri1-card-search" aria-labelledby="liste-b-vege-title">
            <form id="reg-form" class="agri1-form" method="post" action="#1" novalidate>
                ...
                <div class="agri1-grid">
                    ...
                    <div class="agri1-field">
                        ...
                        <select name="cod_mah" class="input_text required" id="cod_mah">
                            <option value="">انتخاب کنید</option>
                            <option value="170" <?php if ($cod_mah == '170') echo 'selected="selected"'; ?>>سیب زمینی</option>
                            <option value="172" <?php if ($cod_mah == '172') echo 'selected="selected"'; ?>>پیاز</option>
                            <option value="174" <?php if ($cod_mah == '174') echo 'selected="selected"'; ?>>گوجه فرنگی</option>
                        </select>
                    </div>
                    <div class="agri1-field">
                        <label class="agri1-label" for="confi2">تاییدیه تکمیلی</label>
                        <select name="confi2" class="input_text required" id="confi2">
                ...
                <p class="agri1-hint">جستجو بدون فیلتر همه را نشان می‌دهد.</p>
...
where  Vege.confi='2' and $v_cod_mah and  Vege_prod.date_ab <= '$date_check' and Vege.mor_cod_m = '$login_session'  and $v_confi2 and $v_add_abadi and $v_add_city and $f_b_time and $f_m_ab and $v_bah_cod_m and $v_z_sal  group by Vege.id LIMIT $start, $limit";
            $query1 = "SELECT COUNT(DISTINCT Vege.id) AS total
FROM Vege
INNER JOIN Vege_prod ON Vege.id = Vege_prod.Vege_id
WHERE Vege.confi = '2' and $v_cod_mah
  ...
  AND $v_z_sal ";
        ...
        <section class="agri1-card agri1-card-results" aria-label="نتایج جستجو">
            ...
            <div class="agri1-export">
                <form action="list_Vege_xls.php" method="post">
                    ...
                    <input type="hidden" name="confi2" value="<?php echo agri2_h($confi2); ?>"/>
                    <input type="hidden" name="cod_mah" value="<?php echo agri2_h($cod_mah); ?>"/>
            ...
            <div class="agri1-table-wrap">
                <table class="agri1-table">
                    ...
                    <thead>
                        <tr>
                            <th class="agri1-ops-col" colspan="2" rowspan="2">عملیات</th>
                            <th rowspan="2">میزان تولید</th>
                            <th rowspan="2">سطح برداشت</th>
                            <th rowspan="2">تاریخ اولین آبیاری</th>
                    ...
                    ?>
                        <tr>
                            <td class="agri1-ops">
                                ...
                                <form action="Vegedata_B_edit.php" method="post" onsubmit="target_po3(this)">
                                    <input type="hidden" name="id" value="<?php echo agri2_h($row['id']); ?>"/>
                                    <button type="submit"><img src="../../files/edit.png" title="ویرایش اطلاعات" width="28" height="22" alt=""/></button>
                                </form>
                                <?php } ?>
                            </td>
                            <td class="agri1-ops">
                                <form action="Vegedata_B_view.php" method="post" onsubmit="target_po3(this)">
                                    <input type="hidden" name="id" value="<?php echo agri2_h($row['id']); ?>"/>
                                    <button type="submit"><img src="../../files/view.png" title="نمایش اطلاعات بهره برداری" width="28" height="22" alt=""/></button>
                                </form>
                            </td>
                            <td>
                            ...
                            <td><?php echo agri2_h($row['date_ab']); ?></td>
                            ...
                            <td><?php echo agri2_h(abadi_name($row['add_abadi'])); ?><?php echo agri2_h(shahr_name($row['add_city'])); ?></td>
        ...
        <nav class="agri1-pager" aria-label="صفحه‌بندی">
            <ul class="agri1-pager-list">
                ...
                <li>
                    <form action="liste_B_Vege.php?id=<?php echo (int)$id - 1; ?>#1" method="post">
                        <?php agri_bvege_filter_hiddens($add_abadi, $add_city, $bah_cod_m, $z_sal, $b_time, $m_ab, $confi2, $cod_mah); ?>
                ...
                <?php if ($show_first) { ?>
                <li>
                    <form action="liste_B_Vege.php?id=1#1" method="post">
                        <?php agri_bvege_filter_hiddens($add_abadi, $add_city, $bah_cod_m, $z_sal, $b_time, $m_ab, $confi2, $cod_mah); ?>
                ...
                <li>
                    ...
                    <?php } else { ?>
                    <form action="liste_B_Vege.php?id=<?php echo (int)$i; ?>#1" method="post">
                        <?php agri_bvege_filter_hiddens($add_abadi, $add_city, $bah_cod_m, $z_sal, $b_time, $m_ab, $confi2, $cod_mah); ?>
                ...
                <li><span class="agri1-pager-ellipsis">...</span></li>
                    <?php } ?>
                <li>
                    <form action="liste_B_Vege.php?id=<?php echo (int)$total; ?>#1" method="post">
                        <?php agri_bvege_filter_hiddens($add_abadi, $add_city, $bah_cod_m, $z_sal, $b_time, $m_ab, $confi2, $cod_mah); ?>
            ...
            <div class="agri1-pager-jump">
                <form id="pageJumpForm" action="liste_B_Vege.php" method="post">
                    <?php agri_bvege_filter_hiddens($add_abadi, $add_city, $bah_cod_m, $z_sal, $b_time, $m_ab, $confi2, $cod_mah); ?>
                    <span>به صفحه</span>
        ...
        (function () {
            ...
            form.addEventListener('submit', function (e) {
                ...
                if (!isNaN(pageId) && pageId >= 1 && pageId <= <?php echo (int)$total; ?>) {
                    this.action = 'liste_B_Vege.php?id=' + pageId + '#1';
                } else {
                    e.preventDefault();
