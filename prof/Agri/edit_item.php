...
if (isset($_POST["id"])) {
    ...
    if (isset($_POST["id_mar"]) && $_POST["id_mar"] !== '') {
        $id_mar = $_POST["id_mar"];
    }
    ...
    // --- بخش اضافه شده: کنترل سطح کشت همزمان در ویرایش ---
    if ($zer_kesht_a > 0 && $zer_kesht_b > 0) {
        echo 'خطا! امکان ثبت همزمان سطح زیر کشت اول و دوم مقدور نیست.';
        exit;
    }
    // --------------------------------------------------
    
    // --- بخش اضافه شده: کنترل سطح برداشت همزمان در ویرایش ---
    if ($s_bar_a > 0 && $s_bar_b > 0) {
        echo 'خطا! امکان ثبت همزمان سطح برداشت اول و دوم مقدور نیست.';
        exit;
    }
    // --------------------------------------------------
    
    // --- بخش اضافه شده: بررسی سطح برداشت نسبت به سطح کشت ---
    if ($s_bar_a > $zer_kesht_a) {
        echo 'خطا! سطح برداشت اول (' . $s_bar_a . ') بزرگتر از سطح زیر کشت اول (' . $zer_kesht_a . ') است.';
        exit;
    }
    
    if ($s_bar_b > $zer_kesht_b) {
        echo 'خطا! سطح برداشت مجدد (' . $s_bar_b . ') بزرگتر از سطح زیر کشت مجدد (' . $zer_kesht_b . ') است.';
        exit;
    }
    // --------------------------------------------------
    ...
    try {
        ...
        $stmt_old = $dbh->prepare("SELECT cod_qroup, cod_mah, zer_kesht_a, zer_kesht_b, id_ostan, id_city, id_mar FROM `$Agri_prod_table` WHERE id=?");
        ...
        if ($id_ostan === '' && !empty($oldData['id_ostan'])) {
            $id_ostan = $oldData['id_ostan'];
        }
        if ($id_city === '' && !empty($oldData['id_city'])) {
            $id_city = $oldData['id_city'];
        }
        if ($id_mar === '' && !empty($oldData['id_mar'])) {
            $id_mar = $oldData['id_mar'];
        }
        
        if (check_payesh($id,0,substr($z_sal, 0, 4)) == 2)
        {
            if (
                $cod_qroup != $oldData['cod_qroup'] ||
                $cod_mah   != $oldData['cod_mah'] ||
                $zer_kesht_a != $oldData['zer_kesht_a'] ||
                $zer_kesht_b != $oldData['zer_kesht_b']
            ) {
                echo 'خطا! برای این رکورد از طریق سامانه پایش ، نهاده اختصاص داده شده و تغییر در گروه، محصول یا سطح زیر کشت مجاز نمی‌باشد.';
        ...
        if ($id_ostan === '' || $id_city === '' || $id_mar === '') {
            echo 'خطا: اطلاعات مکان (استان، شهرستان، مرکز) برای بررسی برش الگوی کشت یافت نشد.';
            throw new Exception("Missing location data for crop pattern validation");
        }
