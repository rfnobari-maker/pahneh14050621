...
$agri_years = array(
    '1405-1406', '1404-1405', '1403-1404', '1402-1403',
    '1401-1402', '1400-1401', '1399-1400', '1398-1399', '1397-1398'
);
...
function manager_agri_ops_svg($name)
{
    $d = array(
...
function manager_agri_has_note($z_sal, $agri_id)
{
    global $dbh;
    $table = 'Agri_note' . str_replace('-', '_', $z_sal);
    try {
        $stmt = $dbh->prepare('SELECT COUNT(*) FROM `' . $table . '` WHERE Agri_id = :agri_id');
        $stmt->bindValue(':agri_id', $agri_id, PDO::PARAM_INT);
        $stmt->execute();
...
function manager_agri_filter_hiddens($z_sal, $bah_cod_m)
{
    $pairs = array(
        'action' => '1',
        'z_sal' => $z_sal,
...
<body class="agri1-body agri1-page">
    ...
    <main class="agri1-main">
        <header class="agri1-hero">
            <h1 class="agri1-title" id="manager-title">مدیریت اطلاعات بهره برداری های زراعی</h1>
        </header>

        <section class="agri1-card agri1-card-search" aria-labelledby="manager-title">
                ...
                <div class="agri1-grid">
                    <div class="agri1-field">
                        ...
                        <select name="z_sal" class="input_text required" id="z_sal">
                            ...
                            <option value="1403-1404" <?php if ($z_sal == '1403-1404') echo 'selected="selected"'; ?>>1403-1404</option>
                            <option value="1402-1403" <?php if ($z_sal == '1402-1403') echo 'selected="selected"'; ?>>1402-1403</option>
                            <option value="1401-1402" <?php if ($z_sal == '1401-1402') echo 'selected="selected"'; ?>>1401-1402</option>
                            <option value="1400-1401" <?php if ($z_sal == '1400-1401') echo 'selected="selected"'; ?>>1400-1401</option>
                            <option value="1399-1400" <?php if ($z_sal == '1399-1400') echo 'selected="selected"'; ?>>1399-1400</option>
                            <option value="1398-1399" <?php if ($z_sal == '1398-1399') echo 'selected="selected"'; ?>>1398-1399</option>
                            <option value="1397-1398" <?php if ($z_sal == '1397-1398') echo 'selected="selected"'; ?>>1397-1398</option>
                        </select>
                    </div>
                    <div class="agri1-field">
...
if (isset($_POST['action'])) {
    ...
    $z_sal = $_POST['z_sal'];
    $found = 0;
    if (in_array($z_sal, $agri_years, true)) {
        ...
        $query = "SELECT s_ayesh,num_bah,id,mor_cod_m,no_mal,bah_cod_m,add_abadi,add_city,sh_gat,z_sal,no_kesh,m_zamin,id_ostan,id_city,t_mah,check_cod from $Agri_table where  bah_cod_m = :bah_cod_m LIMIT $start, $limit";
        $query1 = "SELECT count(*) from $Agri_table where bah_cod_m = :bah_cod_m";
                ...
                <div class="agri1-col-picker" id="agri1-col-picker">
                    ...
                    <div id="agri1-col-panel" class="agri1-col-panel" role="group" aria-labelledby="agri1-col-panel-title">
                        ...
                        <ul class="agri1-col-list">
                            ...
                            <li><label><input type="checkbox" data-col-toggle="s_ayesh" checked/> سطح آیش</label></li>
            ...
            <div class="agri1-table-wrap">
                <table class="agri1-table">
                    <colgroup>
                        ...
                        <col data-col="s_ayesh" style="width:8%"/>
                        <col data-col="m_zamin" style="width:9%"/>
                        <col data-col="no_kesh" style="width:7%"/>
                        <col data-col="no_mal" style="width:10%"/>
                        <col data-col="sh_gat" style="width:7%"/>
                        <col data-col="bah_cod_m" style="width:10%"/>
                        <col data-col="name" style="width:14%"/>
                        <col data-col="abadi" style="width:13%"/>
                    ...
                    <thead>
                        <tr>
                            ...
                            <th data-col="s_ayesh" rowspan="2">سطح آیش</th>
...
<?php     if ($Agri_edit_available == '1') { ?>
                                    <form action="del_list_Agri.php" method="post">
                                        ...
                                        <button type="submit" class="agri1-ops-btn agri1-ops-btn-del" title="حذف اطلاعات زراعی" aria-label="حذف اطلاعات زراعی" onclick="return confirm('از حذف اطلاعات زراعی مطمئن هستید ؟ ')"><?php echo manager_agri_ops_svg('del'); ?></button>
                                    </form>
                                    <form action="P_edit1.php" method="post" onsubmit="target_Agri18(this)">
                                        <input type="hidden" name="bah_cod_m" value="<?php echo agri2_h($row['bah_cod_m']); ?>" />
                                        <input type="hidden" name="id" value="<?php echo agri2_h($row['id']); ?>" />
                                        <input type="hidden" name="z_sal" value="<?php echo agri2_h($z_sal); ?>" />
                                        <button type="submit" class="agri1-ops-btn" title="ویرایش اطلاعات محصول" aria-label="ویرایش اطلاعات محصول"><?php echo manager_agri_ops_svg('crop'); ?></button>
                                    </form>
                                    <form action="Agri_edit.php" method="post">
                                        ...
                                        <input type="hidden" name="id" value="<?php echo agri2_h($row['id']); ?>" />
                                        <button type="submit" class="agri1-ops-btn" title="ویرایش اطلاعات بهره‌برداری" aria-label="ویرایش اطلاعات بهره‌برداری"><?php echo manager_agri_ops_svg('land'); ?></button>
                                    </form>
<?php     } else { ?>
...
<?php     } ?>
                                    <form action="Agri_note" method="post" onsubmit="target_Agri21(this)">
                                        <input type="hidden" name="Agri_id" value="<?php echo agri2_h($row['id']); ?>" />
                                        <input type="hidden" name="z_sal" value="<?php echo agri2_h($z_sal); ?>" />
                                        <input type="hidden" name="mor_cod_m" value="<?php echo agri2_h($login_session); ?>" />
                                        <?php $has_note = manager_agri_has_note($z_sal, $row['id']); ?>
                                        <button type="submit" class="agri1-ops-btn agri1-ops-btn-note<?php echo $has_note ? ' is-has-note' : ''; ?>" title="<?php echo $has_note ? 'نمایش توضیح' : 'درج توضیح'; ?>" aria-label="<?php echo $has_note ? 'نمایش توضیح' : 'درج توضیح'; ?>"><?php echo manager_agri_ops_svg('note'); ?></button>
                                    </form>
                                    <form action="Agridata_view.php" method="post">
                                        <input type="hidden" name="id" value="<?php echo agri2_h($row['id']); ?>" />
                                        <input type="hidden" name="z_sal" value="<?php echo agri2_h($z_sal); ?>" />
                                        <button type="submit" class="agri1-ops-btn" title="نمایش اطلاعات بهره‌برداری" aria-label="نمایش اطلاعات بهره‌برداری"><?php echo manager_agri_ops_svg('view'); ?></button>
                                    </form>
<?php } else { ?>
                                    <form action="Agridata_view.php" method="post">
                                        <input type="hidden" name="id" value="<?php echo agri2_h($row['id']); ?>" />
                                    ...
                                    <span class="agri1-ops-lock" title="اطلاعات بهره‌برداری توسط شما ثبت نشده است" aria-label="اطلاعات بهره‌برداری توسط شما ثبت نشده است"><?php echo manager_agri_ops_svg('lock'); ?></span>
<?php } ?>
                                </div>
                            </td>
                            <td data-col="s_ayesh"><?php echo agri2_h($row['s_ayesh']); ?></td>
                            ...
                            <td class="agri1-name-col" data-col="name"><?php echo agri2_h(str_replace('&nbsp;', ' ', bah_name2($row['bah_cod_m'], $row['num_bah']))); ?></td>
                            <td class="agri1-name-col" data-col="abadi"><?php echo agri2_h(str_replace('&nbsp;', ' ', abadi_name($row['add_abadi']) . shahr_name($row['add_city']))); ?></td>
                            <td class="agri1-name-col" data-col="city"><?php echo agri2_h(str_replace('&nbsp;', ' ', city_name1($row['id_city'], $row['id_ostan']))); ?></td>
...
?>
        <nav class="agri1-pager" aria-label="صفحه‌بندی">
            <ul class="agri1-pager-list">
                <?php if ($id > 1) { ?>
                <li>
                    <form action="manager_Agri.php?id=<?php echo (int)$id - 1; ?>#1" method="post">
                        <?php manager_agri_filter_hiddens($z_sal, $bah_cod_m); ?>
                ...
                <?php if ($show_first) { ?>
                <li>
                    <form action="manager_Agri.php?id=1#1" method="post">
                        <?php manager_agri_filter_hiddens($z_sal, $bah_cod_m); ?>
                ...
                <li>
                    ...
                    <?php } else { ?>
                    <form action="manager_Agri.php?id=<?php echo (int)$i; ?>#1" method="post">
                        <?php manager_agri_filter_hiddens($z_sal, $bah_cod_m); ?>
                ...
                <li><span class="agri1-pager-ellipsis">...</span></li>
                    <?php } ?>
                <li>
                    <form action="manager_Agri.php?id=<?php echo (int)$total; ?>#1" method="post">
                        <?php manager_agri_filter_hiddens($z_sal, $bah_cod_m); ?>
            ...
            <div class="agri1-pager-jump">
                <form id="pageJumpForm" action="manager_Agri.php" method="post">
                    <?php manager_agri_filter_hiddens($z_sal, $bah_cod_m); ?>
                    <span>به صفحه</span>
        ...
        (function () {
            ...
            form.addEventListener('submit', function (e) {
                ...
                if (!isNaN(pageId) && pageId >= 1 && pageId <= <?php echo (int)$total; ?>) {
                    this.action = 'manager_Agri.php?id=' + pageId + '#1';
                } else {
                    e.preventDefault();
...
?>
    ...
    <script>
        ...
        (function () {
            ...
            var KEY = 'manager_Agri_hidden_cols';
            var GROUPS = {
                bah: ['bah_cod_m', 'name'],
                loc: ['abadi', 'city']
            };
            ...
            function apply(map) {
                ...
                for (var g in GROUPS) {
                    if (Object.prototype.hasOwnProperty.call(GROUPS, g)) {
                        visSecond += applyGroup(map, g, GROUPS[g]);
                    }
                }
