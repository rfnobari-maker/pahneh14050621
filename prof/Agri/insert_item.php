<?php
// ============================================
// غیرفعال کردن نمایش خطاهای PHP (برای محیط تولید)
// ============================================
error_reporting(0);
...
// بررسی وجود کلاس PreviousYearClearanceChecker
if (file_exists('PreviousYearClearanceChecker.php')) {
    require_once('PreviousYearClearanceChecker.php');
    $hasClearanceChecker = true;
} else {
    $hasClearanceChecker = false;
}
...
if (isset($_POST["f_cod_qroup"])) {
    ...
    // بررسی تعیین‌تکلیف سال قبل (فقط برای زراعی)
    if ($hasClearanceChecker) {
    ...
    foreach ($cod_qroup as $count => $value) {
        // اعتبارسنجی سطح کشت (فقط یکی از دو مقدار باید > 0 باشد)
        ...
        // اعتبارسنجی سطح برداشت (فقط یکی از دو مقدار باید > 0 باشد)
        ...
        // اعتبارسنجی سطح برداشت نسبت به سطح کشت
        $zk_a = (float)$zer_kesht_a[$count];
        $zk_b = (float)$zer_kesht_b[$count];
        $sb_a = (float)$s_bar_a[$count];
        $sb_b = (float)$s_bar_b[$count];
        
        if ($sb_a > 0 && $sb_a > $zk_a) {
            $error_messages[] = "محصول در ردیف " . ($count + 1) . ": سطح برداشت اول (" . $sb_a . ") بزرگتر از سطح زیر کشت اول (" . $zk_a . ") است.";
            continue;
        }
        
        if ($sb_b > 0 && $sb_b > $zk_b) {
            $error_messages[] = "محصول در ردیف " . ($count + 1) . ": سطح برداشت مجدد (" . $sb_b . ") بزرگتر از سطح زیر کشت مجدد (" . $zk_b . ") است.";
            continue;
        }
        
        // شرط بررسی فیلدهای خالی
        if (
            ...
            $final_product_code_for_sum = $current_product_code;
            if ($z_sal === '1404-1405' && isset($product_map[$current_product_code])) {
                $final_product_code_for_sum = $product_map[$current_product_code];
            }
