...
function liste_agri_ops_svg($name)
{
    $d = array(
...
function liste_agri_has_note($z_sal, $agri_id)
{
    global $dbh;
...
function liste_agri_filter_hiddens($add_abadi, $add_city, $no_mal, $no_kesh, $bah_cod_m, $m_cod_m, $z_sal, $t_mah, $docId)
{
    $pairs = array(
        'action_lise' => '1',
        'add_abadi' => $add_abadi,
...
<body class="agri1-body agri1-page">
    ...
    <div id="message-box" dir="rtl">
        <p>تمامی مسئولیت‌های اطلاعات ثبت شده بر عهده کارشناس پهنه می‌باشد. به همین منظور، در بخش زراعت، آیکون «لیست حذفی‌های زراعی» اضافه شده است و در صورت لزوم، تنها همین لیست در اختیار نهادهای نظارتی قرار خواهد گرفت.</p>
        <p>مجدداً تأکید می‌شود از پذیرش ثبت، حذف و ویرایش دستوری اطلاعات توسط هر مقام و مسئولی جداً خودداری فرمایید.</p>
        <label>
            <input type="checkbox" id="dont-show-again"/> مطالعه کردم ، این پیام مجددا نمایش داده نشود
        </label>
        <button type="button" id="close-message">بستن</button>
    </div>

    <main class="agri1-main">
        <header class="agri1-hero">
            <h1 class="agri1-title" id="liste-title">لیست بهره‌برداری‌های زراعی</h1>
        </header>
        ...
        <div class="agri1-col-picker" id="agri1-col-picker">
            ...
            <div id="agri1-col-panel" class="agri1-col-panel" role="group" aria-labelledby="agri1-col-panel-title">
                ...
                <ul class="agri1-col-list">
                    ...
                    <li><label><input type="checkbox" data-col-toggle="m_cod_m" checked/> کد ملی مالک</label></li>
                    ...
                    <li><label><input type="checkbox" data-col-toggle="tel" checked/> همراه</label></li>
...
<form  action="list_Agri_xls.php" method="post" class="agri1-xls">
        ...
        <input type="hidden" name="m_cod_m"   value="<?php echo agri2_h($m_cod_m); ?>" />
...
<div class="agri1-table-wrap">
    <table class="agri1-table">
        <colgroup>
            ...
            <col data-col="s_ayesh" style="width:6%"/>
            <col data-col="m_zamin" style="width:7%"/>
            <col data-col="no_kesh" style="width:6%"/>
            <col data-col="m_cod_m" style="width:9%"/>
            <col data-col="no_mal" style="width:8%"/>
            <col data-col="sh_gat" style="width:6%"/>
            <col data-col="tel" style="width:8%"/>
            <col data-col="bah_cod_m" style="width:9%"/>
        ...
        <thead>
            <tr>
                ...
                <th data-col="m_cod_m" rowspan="2">کد ملی مالک</th>
                <th data-col="no_mal" rowspan="2">نوع مالکیت</th>
                <th data-col="sh_gat" rowspan="2">شماره قطعه</th>
                <th data-col-group="bah" colspan="3">مشخصات بهره‌بردار</th>
                <th data-col="abadi" rowspan="2">آبادی/شهر</th>
                <th data-col="rownum" rowspan="2">ردیف</th>
            </tr>
            <tr>
                <th data-col="tel">همراه</th>
        ...
        ?>
            ...
            <?php if ($Agri_edit_available == '1') { ?>
                    <form action="del_list_Agri.php" method="post">
                        ...
                        <button type="submit" class="agri1-ops-btn agri1-ops-btn-del" title="حذف اطلاعات زراعی" aria-label="حذف اطلاعات زراعی" onclick="return confirm('از حذف اطلاعات زراعی مطمئن هستید ؟ ')"><?php echo liste_agri_ops_svg('del'); ?></button>
                    </form>
                    <form action="P_edit1.php" method="post" onsubmit="target_Agri18(this)">
                        <input type="hidden" name="bah_cod_m" value="<?php echo agri2_h($row['bah_cod_m']); ?>" />
                        <input type="hidden" name="id" value="<?php echo agri2_h($row['id']); ?>" />
                        <input type="hidden" name="z_sal" value="<?php echo agri2_h($z_sal); ?>" />
                        <button type="submit" class="agri1-ops-btn" title="ویرایش اطلاعات محصول" aria-label="ویرایش اطلاعات محصول"><?php echo liste_agri_ops_svg('crop'); ?></button>
                    </form>
                    <form action="Agri_edit.php" method="post">
                        ...
                        <input type="hidden" name="id" value="<?php echo agri2_h($row['id']); ?>" />
                        <input type="hidden" name="id_page" value="<?php echo (int)$id; ?>" />
                        <button type="submit" class="agri1-ops-btn" title="ویرایش اطلاعات زمین" aria-label="ویرایش اطلاعات زمین"><?php echo liste_agri_ops_svg('land'); ?></button>
                    </form>
            <?php } else { ?>
            ...
            <?php } ?>
                    <form action="Agri_note" method="post" onsubmit="target_Agri21(this)">
                        ...
                        <?php $has_note = liste_agri_has_note($z_sal, $row['id']); ?>
                        <button type="submit" class="agri1-ops-btn agri1-ops-btn-note<?php echo $has_note ? ' is-has-note' : ''; ?>" title="<?php echo $has_note ? 'نمایش توضیح' : 'درج توضیح'; ?>" aria-label="<?php echo $has_note ? 'نمایش توضیح' : 'درج توضیح'; ?>"><?php echo liste_agri_ops_svg('note'); ?></button>
                    </form>
                <form action="Agridata_view.php" method="post">
                    <input type="hidden" name="id" value="<?php echo agri2_h($row['id']); ?>" />
                    <input type="hidden" name="z_sal" value="<?php echo agri2_h($z_sal); ?>" />
                    <input type="hidden" name="id_page" value="<?php echo (int)$id; ?>" />
                    <button type="submit" class="agri1-ops-btn" title="نمایش اطلاعات بهره‌برداری" aria-label="نمایش اطلاعات بهره‌برداری"><?php echo liste_agri_ops_svg('view'); ?></button>
                </form>
                </div>
            </td>
            <td data-col="s_ayesh"><?php echo agri2_h($row['s_ayesh'] * 1); ?></td>
            <td data-col="m_zamin"><?php echo agri2_h($row['m_zamin'] * 1); ?></td>
            <td data-col="no_kesh"><?php echo agri2_h($v_no_kesh); ?></td>
            <td data-col="m_cod_m" dir="ltr"><?php echo agri2_h($row['m_cod_m']); ?></td>
            <td data-col="no_mal"><?php echo agri2_h($v_no_mal); ?></td>
            <td data-col="sh_gat"><?php echo agri2_h($row['sh_gat']); ?></td>
            <td data-col="tel" dir="ltr"><?php echo agri2_h(bah_tel_m($row['bah_cod_m'])); ?></td>
            <td data-col="bah_cod_m" dir="ltr"><?php echo agri2_h($row['bah_cod_m']); ?></td>
            <td class="agri1-name-col" data-col="name"><?php echo agri2_h(trim($row['last_name'] . ' ' . $row['name'])); ?></td>
    ...
    ?>
        <nav class="agri1-pager" aria-label="صفحه‌بندی">
            <ul class="agri1-pager-list">
                ...
                <li>
                    <form action="liste_Agri.php?id=<?php echo (int)$id - 1; ?>#1" method="post">
                        <?php liste_agri_filter_hiddens($add_abadi, $add_city, $no_mal, $no_kesh, $bah_cod_m, $m_cod_m, $z_sal, $t_mah, $docId); ?>
                ...
                <?php if ($show_first) { ?>
                <li>
                    <form action="liste_Agri.php?id=1#1" method="post">
                        <?php liste_agri_filter_hiddens($add_abadi, $add_city, $no_mal, $no_kesh, $bah_cod_m, $m_cod_m, $z_sal, $t_mah, $docId); ?>
                ...
                <li>
                    ...
                    <?php } else { ?>
                    <form action="liste_Agri.php?id=<?php echo (int)$i; ?>#1" method="post">
                        <?php liste_agri_filter_hiddens($add_abadi, $add_city, $no_mal, $no_kesh, $bah_cod_m, $m_cod_m, $z_sal, $t_mah, $docId); ?>
                ...
                <li><span class="agri1-pager-ellipsis">...</span></li>
                    <?php } ?>
                <li>
                    <form action="liste_Agri.php?id=<?php echo (int)$total; ?>#1" method="post">
                        <?php liste_agri_filter_hiddens($add_abadi, $add_city, $no_mal, $no_kesh, $bah_cod_m, $m_cod_m, $z_sal, $t_mah, $docId); ?>
            ...
            <div class="agri1-pager-jump">
                <form id="pageJumpForm" action="liste_Agri.php" method="post">
                    <?php liste_agri_filter_hiddens($add_abadi, $add_city, $no_mal, $no_kesh, $bah_cod_m, $m_cod_m, $z_sal, $t_mah, $docId); ?>
                    <span>به صفحه</span>
...
?>
    ...
    <script>
        ...
        (function () {
            ...
            var KEY = 'liste_Agri_hidden_cols';
            var GROUP = { bah: ['tel', 'bah_cod_m', 'name'] };
            ...
            function apply(map) {
                ...
                var groupTh = table.querySelector('[data-col-group="bah"]');
                if (groupTh) {
                    var vis = 0;
                    var kids = GROUP.bah;
                    ...
                    if (vis === 0) {
                        ...
                        if (groupRow) groupRow.classList.add('is-col-hidden');
                    } else {
                        groupTh.classList.remove('is-col-hidden');
                        groupTh.colSpan = vis;
                        if (groupRow) groupRow.classList.remove('is-col-hidden');
                    }
                }
