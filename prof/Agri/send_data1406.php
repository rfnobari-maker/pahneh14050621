...
if(isset($_POST['Go_send_data'])){
	...
	`Agri1405_1406` where  bah_cod_m = :bah_cod_m  and mor_cod_m = :mor_cod_m"; 
$stmt = $dbh->prepare($query);
$stmt->execute(array(':bah_cod_m'=>$bah_cod_m,':mor_cod_m'=>$login_session));
$found = $stmt -> rowCount();
    if ($found>0) {
         $com_alert = 'خطا : \n \n برای بهره بردار قبلا اطلاعات زراعی 1406-1405 ثبت شده است' ;
    }
else 
{
	foreach($idArr as $id){
$query ="CREATE TEMPORARY TABLE tmp$id SELECT * FROM `Agri1404_1405` WHERE bah_cod_m=:bah_cod_m and mor_cod_m = :mor_cod_m and id=:id" ;
$stmt = $dbh->prepare($query);
$stmt->execute(array(':bah_cod_m'=>$bah_cod_m,':mor_cod_m'=>$mor_cod_m,':id'=>$id));
$query = "update tmp$id set date_s = :date_s ,id = '',z_sal='1405-1406',t_mah=0,s_ayesh=0 where bah_cod_m = :bah_cod_m and mor_cod_m = :mor_cod_m ";
$stmt = $dbh->prepare($query);
$stmt->execute(array(':date_s'=>$date_edit,':bah_cod_m'=>$bah_cod_m,':mor_cod_m'=>$mor_cod_m));
$query = "INSERT INTO `Agri1405_1406` SELECT * FROM tmp$id" ;  	
$stmt = $dbh->prepare($query);
$stmt->execute();
//
//
	//sabt_event($login_session,$_SERVER['REMOTE_ADDR'],$date_edit,$time,'','حذف پیام دریافتی از/ '.user_name($s_user)) ; 
         ...
         sabt_event($login_session,getUserIP_1(),$date_edit,$time,'','انتقال اطلاعات پایه زراعی 1404-1404 / '.$bah_cod_m,$id_ostan) ; 
	     $com_alert = 'اطلاعات با موفقیت ارسال شد.   \n \n اکنون می توانید نسبت به ویرایش اطلاعات 1406-1405 اقدام فرمایید ';
 }
 }
