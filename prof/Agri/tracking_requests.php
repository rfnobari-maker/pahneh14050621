...
function agri_track_filter_hiddens($z_sal, $bah_cod_m)
{
    $pairs = array(
        'action' => '1',
        'z_sal' => $z_sal,
...
if (!empty($bah_cod_m)) {
    $where .= " AND bah_cod_m = :bah_cod_m";
}

$count_sql = "SELECT COUNT(*) FROM Agri_prod_req $where";
...
$sql = "SELECT * FROM Agri_prod_req $where ORDER BY id DESC LIMIT $start, $limit";
...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .agri1-table th.agri1-col-num { width: 72px; }
...
<body class="agri1-body agri1-page">
    ...
    <main class="agri1-main" id="1">
        <header class="agri1-hero">
            <h1 class="agri1-title" id="agri-track-title">پیگیری درخواست‌های اصلاح</h1>
        </header>

        <section class="agri1-card agri1-card-search" aria-labelledby="agri-track-title">
                ...
                <table class="agri1-table">
                    <thead>
                        <tr>
                            ...
                            <th>نام محصول<br>(قدیم ← جدید)</th>
                            <th class="agri1-col-num">سطح زیر کشت اول<br>(قدیم ← جدید)</th>
                            <th class="agri1-col-num">سطح زیر کشت دوم<br>(قدیم ← جدید)</th>
                            <th class="agri1-col-num">پیش بینی تولید<br>(قدیم ← جدید)</th>
                            <th>علت درخواست و نظرات</th>
                            <th class="agri1-col-status">وضعیت</th>
                            <th class="agri1-ops-col">حذف درخواست</th>
                        </tr>
                    </thead>
                    <tbody>
<?php
...
?>
                        <tr>
                            ...
                            <td class="agri1-name-col">
                                <strong><?php echo agri2_h(str_replace('&nbsp;', ' ', $farmer_name)); ?></strong><br>
                                <small><?php echo agri2_h($row['bah_cod_m']); ?></small>
                            </td>
                            <td>
                                <span class="agri1-old-val"><?php echo agri2_h(mah_name($row['cod_mah'])); ?></span>
                                <span class="agri1-new-val"><?php echo agri2_h(mah_name($row['new_cod_mah'])); ?></span>
                            </td>
                            <td>
                                <span class="agri1-old-val"><?php echo agri2_h($row['zer_kesht_a']); ?></span>
                                <span class="agri1-new-val"><?php echo agri2_h($row['new_zer_kesht_a']); ?></span>
                            </td>
                            <td>
                                <span class="agri1-old-val"><?php echo agri2_h($row['zer_kesht_b']); ?></span>
                                <span class="agri1-new-val"><?php echo agri2_h($row['new_zer_kesht_b']); ?></span>
                            </td>
                            <td>
                                <span class="agri1-old-val"><?php echo agri2_h($row['mah_tolp']); ?></span>
                                <span class="agri1-new-val"><?php echo agri2_h($row['new_mah_tolp']); ?></span>
                            </td>
...
?>
        <nav class="agri1-pager" aria-label="صفحه‌بندی">
            <ul class="agri1-pager-list">
                <?php if ($id > 1) { ?>
                <li>
                    <form action="tracking_requests.php?id=<?php echo (int)$id - 1; ?>#1" method="post">
                        <?php agri_track_filter_hiddens($z_sal, $bah_cod_m); ?>
                ...
                <?php if ($show_first) { ?>
                <li>
                    <form action="tracking_requests.php?id=1#1" method="post">
                        <?php agri_track_filter_hiddens($z_sal, $bah_cod_m); ?>
                ...
                <li>
                    ...
                    <?php } else { ?>
                    <form action="tracking_requests.php?id=<?php echo (int)$i; ?>#1" method="post">
                        <?php agri_track_filter_hiddens($z_sal, $bah_cod_m); ?>
                ...
                <li><span class="agri1-pager-ellipsis">...</span></li>
                    <?php } ?>
                <li>
                    <form action="tracking_requests.php?id=<?php echo (int)$total; ?>#1" method="post">
                        <?php agri_track_filter_hiddens($z_sal, $bah_cod_m); ?>
            ...
            <div class="agri1-pager-jump">
                <form id="pageJumpForm" action="tracking_requests.php" method="post">
                    <?php agri_track_filter_hiddens($z_sal, $bah_cod_m); ?>
                    <span>به صفحه</span>
        ...
        (function () {
            ...
            form.addEventListener('submit', function (e) {
                ...
                if (!isNaN(pageId) && pageId >= 1 && pageId <= <?php echo (int)$total; ?>) {
                    this.action = 'tracking_requests.php?id=' + pageId + '#1';
                } else {
                    e.preventDefault();
...
?>
        ...
        <p class="agri1-back">
            <a class="agri1-btn agri1-btn-ghost" href="Agri_reg.php" title="برگشت به صفحه قبل">
