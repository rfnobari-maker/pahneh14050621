...
class PreviousYearClearanceChecker {
     ...
     */
    private $default_excluded_products = array('144', '108', '136', '172', '170','128','112','172','170','120','208','156','202');
    ...
    private function checkAgriPreviousYear() {
        ...
        $query = "SELECT p.id, p.cod_mah, p.mah_tol, p.mah_kh, p.z_sal,
                  ...
                  AND (COALESCE(p.zer_kesht_a, 0) + COALESCE(p.zer_kesht_b, 0)) > 0";
    ...
    private function checkVegePreviousYear() {
        ...
        $query = "SELECT p.id, p.cod_mah, p.z_sal, p.Vege_id, p.zer_kesht
                  FROM Vege_prod p
                  WHERE p.bah_cod_m = :bah_cod_m 
                  AND p.mor_cod_m = :mor_cod_m
                  AND p.z_sal = :z_sal
