function edit_row(id)
{
 ...
 var num_spawn =document.getElementById("num_spawn_val"+id).innerHTML;
 ...
 document.getElementById("num_spawn_val"+id).innerHTML="<input type='text' style='width:70px; height:30px ; ' id='num_spawn_text"+id+"' value='"+num_spawn+"'>";
...
function save_row(id)
{
 ...
 var num_spawn=document.getElementById("num_spawn_text"+id).value;
 var zer_kesh = num_row * num_t_row * h_t * w_t * num_spawn ; 
 ...
 ({
  ...
  success:function(response) {
   if(response=="success")
   {
    ...
    document.getElementById("num_spawn_val"+id).innerHTML=num_spawn;
...
function insert_row(unit_id,y_prod)
{
 ...
 var num_spawn=document.getElementById("new_num_spawn").value;
 var zer_kesh = num_row * num_t_row * h_t * w_t  * num_spawn; 
 var zer_kesh = Math.round(zer_kesh*100) / 100 ; 
 $.ajax
 ({
  ...
  success:function(response) {
   if(response!="")
   {
    ...
    var row = table.insertRow(table_len).outerHTML="<tr id='row"+id+"'><td id='num_row_val"+id+"'>"+num_row+"</td><td id='num_t_row_val"+id+"'>"+num_t_row+"</td><td id='num_h_t_val"+id+"'>"+h_t+"</td><td id='w_t_val"+id+"'>"+w_t+"</td><td id='num_spawn_val"+id+"'>"+num_spawn+"</td><td id='zer_kesh_val"+id+"'>"+zer_kesh+"</td><td><input type='button' class='edit_button' id='edit_button"+id+"' value='ویرایش' onclick='edit_row("+id+");'/><input type='button' class='save_button' id='save_button"+id+"' value='ذخیره' onclick='save_row("+id+");'/><input type='button' class='delete_button' id='delete_button"+id+"' value='حذف' onclick='delete_row("+id+");'/></td></tr>";
    ...
    document.getElementById("new_num_spawn").value="";
   }
  }
 });
}