...
function manager_garden_ops_svg($name)
{
    $d = array(
...
function manager_garden_has_note($z_sal, $garden_id)
{
    global $dbh;
    $table = 'Garden_note' . $z_sal;
    try {
        $stmt = $dbh->prepare('SELECT COUNT(*) FROM `' . $table . '` WHERE Garden_id = :garden_id');
        $stmt->bindValue(':garden_id', $garden_id, PDO::PARAM_INT);
        $stmt->execute();
...
function manager_garden_filter_hiddens($z_sal, $bah_cod_m)
{
    $pairs = array(
        'action' => '1',
        'z_sal' => $z_sal,
...
<body class="agri1-body agri1-page">
    ...
    <main class="agri1-main">
        <header class="agri1-hero">
            <h1 class="agri1-title" id="manager-title">مدیریت اطلاعات بهره برداری های باغی و قلمستان</h1>
        </header>
...
if (isset($_POST['action'])) {
    ...
    $z_sal = $_POST['z_sal'];
    $found = 0;
    if (in_array($z_sal, $garden_years, true)) {
        $Garden_edit_available = ($z_sal == '1405') ? '1' : '0';
        $can_note = ($z_sal > '1404');
        $query = "SELECT id,no_mal,no_kesh,nah_kesh,mor_cod_m,add_abadi,add_city,sh_gat,z_sal,t_mah,id_city,id_ostan,bah_cod_m,num_bah,m_zamin,check_cod from Garden where bah_cod_m = :bah_cod_m and z_sal = :z_sal LIMIT $start, $limit";
        $query1 = "SELECT count(*) from Garden where bah_cod_m = :bah_cod_m and z_sal = :z_sal";
                ...
                <div class="agri1-col-picker" id="agri1-col-picker">
                    ...
                    <div id="agri1-col-panel" class="agri1-col-panel" role="group" aria-labelledby="agri1-col-panel-title">
                        ...
                        <ul class="agri1-col-list">
                            ...
                            <li><label><input type="checkbox" data-col-toggle="nah_kesh" checked/> نحوه کشت</label></li>
            ...
            <div class="agri1-table-wrap">
                <table class="agri1-table">
                    <colgroup>
                        ...
                        <col data-col="nah_kesh" style="width:8%"/>
                    ...
                    <thead>
                        <tr>
                            ...
                            <th data-col="nah_kesh" rowspan="2">نحوه کشت</th>
...
<?php
        ...
        foreach ($stmt as $row) {
            ...
            $row_z_sal = $row['z_sal'];
            $has_note = $can_note ? manager_garden_has_note($row_z_sal, $row['id']) : false;
?>
                        <tr>
                            <td class="agri1-ops" data-col="ops">
                                <div class="agri1-ops-bar">
<?php if ($row['mor_cod_m'] == $login_session) { ?>
<?php     if ($Garden_edit_available == '1') { ?>
                                    <form action="del_list_Garden.php" method="post">
                                        ...
                                        <input type="hidden" name="check_cod" value="<?php echo agri2_h($row['check_cod']); ?>" />
                                        <input type="hidden" name="id_page" value="<?php echo (int)$id; ?>" />
                                        <button type="submit" class="agri1-ops-btn agri1-ops-btn-del" title="حذف اطلاعات باغی" aria-label="حذف اطلاعات باغی" onclick="return confirm('از حذف اطلاعات باغی و قلمستان مطمئن هستید ؟ ')"><?php echo manager_garden_ops_svg('del'); ?></button>
                                    </form>
                                    <form action="P_edit1.php" method="post" onsubmit="target_Agri18(this)">
                                        <input type="hidden" name="bah_cod_m" value="<?php echo agri2_h($row['bah_cod_m']); ?>" />
                                        <input type="hidden" name="id" value="<?php echo agri2_h($row['id']); ?>" />
                                        <input type="hidden" name="z_sal" value="<?php echo agri2_h($row_z_sal); ?>" />
                                        <input type="hidden" name="from_page" value="manager" />
                                        <button type="submit" class="agri1-ops-btn" title="ویرایش اطلاعات محصول" aria-label="ویرایش اطلاعات محصول"><?php echo manager_garden_ops_svg('crop'); ?></button>
                                    </form>
                                    <form action="Garden_edit.php" method="post">
                                        ...
                                        <input type="hidden" name="id" value="<?php echo agri2_h($row['id']); ?>" />
                                        <button type="submit" class="agri1-ops-btn" title="ویرایش اطلاعات زمین" aria-label="ویرایش اطلاعات زمین"><?php echo manager_garden_ops_svg('land'); ?></button>
                                    </form>
<?php     } else { ?>
...
<?php     if ($can_note) { ?>
                                    <form action="Garden_note" method="post" onsubmit="target_Agri21(this)">
                                        ...
                                        <button type="submit" class="agri1-ops-btn agri1-ops-btn-note<?php echo $has_note ? ' is-has-note' : ''; ?>" title="<?php echo $has_note ? 'نمایش توضیح' : 'درج توضیح'; ?>" aria-label="<?php echo $has_note ? 'نمایش توضیح' : 'درج توضیح'; ?>"><?php echo manager_garden_ops_svg('note'); ?></button>
                                    </form>
<?php     } ?>
                                    <form action="Gardendata_manager.php" method="post">
                                        <input type="hidden" name="id" value="<?php echo agri2_h($row['id']); ?>" />
                                        <button type="submit" class="agri1-ops-btn" title="نمایش اطلاعات بهره‌برداری" aria-label="نمایش اطلاعات بهره‌برداری"><?php echo manager_garden_ops_svg('view'); ?></button>
                                    </form>
<?php } else { ?>
                                    <form action="Gardendata_manager.php" method="post">
                                        <input type="hidden" name="id" value="<?php echo agri2_h($row['id']); ?>" />
...
<?php     } ?>
                                    <span class="agri1-ops-lock" title="اطلاعات بهره‌برداری توسط شما ثبت نشده است" aria-label="اطلاعات بهره‌برداری توسط شما ثبت نشده است"><?php echo manager_garden_ops_svg('lock'); ?></span>
<?php } ?>
                                </div>
                            </td>
                            ...
                            <td data-col="nah_kesh"><?php echo agri2_h($v_nah_kesh); ?></td>
...
if ($query1 !== '') {
    ...
    $stmt1->execute(array(':bah_cod_m' => $bah_cod_m, ':z_sal' => $z_sal));
...
?>
        <nav class="agri1-pager" aria-label="صفحه‌بندی">
            <ul class="agri1-pager-list">
                <?php if ($id > 1) { ?>
                <li>
                    <form action="manager_Garden.php?id=<?php echo (int)$id - 1; ?>#1" method="post">
                        <?php manager_garden_filter_hiddens($z_sal, $bah_cod_m); ?>
                ...
                <?php if ($show_first) { ?>
                <li>
                    <form action="manager_Garden.php?id=1#1" method="post">
                        <?php manager_garden_filter_hiddens($z_sal, $bah_cod_m); ?>
                ...
                <li>
                    ...
                    <?php } else { ?>
                    <form action="manager_Garden.php?id=<?php echo (int)$i; ?>#1" method="post">
                        <?php manager_garden_filter_hiddens($z_sal, $bah_cod_m); ?>
                ...
                <li><span class="agri1-pager-ellipsis">...</span></li>
                    <?php } ?>
                <li>
                    <form action="manager_Garden.php?id=<?php echo (int)$total; ?>#1" method="post">
                        <?php manager_garden_filter_hiddens($z_sal, $bah_cod_m); ?>
            ...
            <div class="agri1-pager-jump">
                <form id="pageJumpForm" action="manager_Garden.php" method="post">
                    <?php manager_garden_filter_hiddens($z_sal, $bah_cod_m); ?>
                    <span>به صفحه</span>
        ...
        (function () {
            ...
            form.addEventListener('submit', function (e) {
                ...
                if (!isNaN(pageId) && pageId >= 1 && pageId <= <?php echo (int)$total; ?>) {
                    this.action = 'manager_Garden.php?id=' + pageId + '#1';
                } else {
                    e.preventDefault();
...
?>
    ...
    <script>
        ...
        (function () {
            ...
            var KEY = 'manager_Garden_hidden_cols';
            var GROUPS = {
                bah: ['bah_cod_m', 'name'],
                loc: ['abadi', 'city']
            };
