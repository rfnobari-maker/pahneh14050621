...
$stmt->execute(array(':bah_cod_m'=>$bah_cod_m,':sal'=>'1398',':mor_cod_m'=>$login_session));
$found = $stmt -> rowCount();
    if ($found>0) {
         $com_alert = 'خطا : \n \n برای بهره بردار قبلا اطلاعات گلخانه سال 1398 ثبت شده است' ;
    }
else 
{
		foreach($idArr as $id){
...
$stmt->execute();
	//sabt_event($login_session,getUserIP_1(),$date_edit,$time,'','حذف پیام دریافتی از/ '.user_name($s_user)) ; 
	     ...
	     $com_alert = 'اطلاعات با موفقیت ارسال شد.   \n \n اکنون می توانید نسبت به ویرایش اطلاعات 1398 اقدام فرمایید ';
 }
 }
...
<form name="myform1" class="myform" method="post" action="liste_Greenhous.php#1">
        ...
        <input type="hidden" name="sal" value="1398" />
