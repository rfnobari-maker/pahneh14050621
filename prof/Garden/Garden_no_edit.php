...
function agri_noedit_filter_hiddens($id_ostan1, $id_city, $id_mar, $add_abadi, $add_city, $bah_cod_m, $mor_cod_m, $no_kesh, $z_sal, $mah_qroup, $mah_name, $dis)
{
    $pairs = array(
        'action' => '1',
        'id_ostan' => $id_ostan1,
...
<body class="agri1-body agri1-page">
    ...
    $pahneh_crumb = array(
        ...
        array('label' => 'ویرایش نشده های بعد از انتقال'),
    );
    ...
    <main class="agri1-main">
        <header class="agri1-hero">
            <h1 class="agri1-title" id="garden-noedit-title">ویرایش نشده های بعد از انتقال</h1>
        </header>

        <section class="agri1-card agri1-card-search" aria-labelledby="garden-noedit-title">
                ...
                <div class="agri1-grid">
                    <div class="agri1-field">
                        ...
                        <select name="z_sal" class="input_text required" id="z_sal" tabindex="1">
                            ...
                            foreach ($stmt as $row) {
                            ?>
                            <option value="<?php echo agri2_h($row['sal']); ?>"
                                <?php if (isset($z_sal) && $row['sal'] == $z_sal) echo 'selected="selected"'; ?>> <?php echo agri2_h($row['sal']); ?></option>
                            <?php } ?>
                        </select>
                    </div>
                    <div class="agri1-field">
                    ...
                    <div class="agri1-field">
                        ...
                        <select name="add_abadi" class="input_text" id="add_abadi" tabindex="5" dir="rtl">
                            <option value="0">انتخاب کنید</option>
                            <?php
                            $query = "SELECT add_abadi,abadi FROM list_abadi WHERE id_ostan = '$id_ostan1' and id_city = '$id_city' and id_mar = '$id_mar' and `mor_cod_m` = '$login_session' ORDER BY BINARY abadi";
                            $stmt = $dbh->prepare($query);
                            $stmt->execute();
                            foreach ($stmt as $row) {
                            ?>
                    ...
                    <div class="agri1-field">
                        ...
                        <select name="add_city" class="input_text" id="add_city" tabindex="6" dir="rtl">
                            <option value="0">انتخاب کنید</option>
                            <?php
                            $query = "SELECT add_city,shahr FROM list_city  WHERE id_ostan = '$id_ostan1' and id_city = '$id_city' and id_mar = '$id_mar' and `mor_cod_m` = '$login_session'  ORDER BY BINARY shahr";
                            $stmt = $dbh->prepare($query);
                            $stmt->execute();
                            foreach ($stmt as $row) {
                            ?>
        ...
        if (isset($_POST['action'])) {
            ...
            if ($id < 1) $id = 1;
            $start = ($id - 1) * $limit;
            $query = "SELECT Garden.id, Garden.num_bah, Garden.mor_cod_m, Garden.no_mal, Garden.bah_cod_m, Garden.add_abadi, Garden.add_city, Garden.sh_gat, Garden.z_sal, Garden.no_kesh, Garden.nah_kesh, Garden.m_zamin, Garden.id_ostan, Garden.id_city, Garden.id_mar, Garden.t_mah from Garden
 where Garden.date_s not like '%/%' and  $v_id_ostan  and $v_id_city and  $v_id_mar and $f_add_abadi and $f_add_city and $f_no_kesh  and $v_mor_cod_m and $v_bah_cod_m and  $v_z_sal and  $v_cod_mah
  ORDER BY Garden.bah_cod_m ASC LIMIT $start, $limit ";
            $query1 = "SELECT count(*) from Garden where Garden.date_s not like '%/%' and  $v_id_ostan  and $v_id_city and  $v_id_mar and $f_add_abadi and $f_add_city and $f_no_kesh  and $v_mor_cod_m and $v_bah_cod_m and  $v_z_sal and $v_cod_mah";
            $stmt = $dbh->prepare($query);
            $stmt->execute();
            $t_row = $stmt->rowCount();
            if ($t_row > 0) {
        ...
        <section class="agri1-card agri1-card-results" aria-label="نتایج جستجو">
            <div class="agri1-results-toolbar">
                ...
                <div class="agri1-xls">
                    <form action="Garden_no_edit_xls.php" method="post">
                        <input type="hidden" name="id_ostan" value="<?php echo agri2_h($id_ostan1); ?>"/>
                        <input type="hidden" name="id_city5" value="<?php echo agri2_h($id_city); ?>"/>
                        ...
                        <input type="hidden" name="mah_qroup" value="<?php echo agri2_h($mah_qroup); ?>"/>
                        <input type="hidden" name="mah_name" value="<?php echo agri2_h($mah_name); ?>"/>
                        <input type="hidden" name="dis" value="<?php echo agri2_h($dis); ?>"/>
                    ...
                    ?>
                        <tr>
                            <td class="agri1-ops" data-col="ops">
                                <form action="Gardendata_view1.php" method="post" onsubmit="target_Gar15(this)">
        ...
        <nav class="agri1-pager" aria-label="صفحه‌بندی">
            <ul class="agri1-pager-list">
                ...
                <li>
                    <form action="Garden_no_edit.php?id=<?php echo (int)$id - 1; ?>#1" method="post">
                        <?php agri_noedit_filter_hiddens($id_ostan1, $id_city, $id_mar, $add_abadi, $add_city, $bah_cod_m, $mor_cod_m, $no_kesh, $z_sal, $mah_qroup, $mah_name, $dis); ?>
                ...
                <?php if ($show_first) { ?>
                <li>
                    <form action="Garden_no_edit.php?id=1#1" method="post">
                        <?php agri_noedit_filter_hiddens($id_ostan1, $id_city, $id_mar, $add_abadi, $add_city, $bah_cod_m, $mor_cod_m, $no_kesh, $z_sal, $mah_qroup, $mah_name, $dis); ?>
                ...
                <li>
                    ...
                    <?php } else { ?>
                    <form action="Garden_no_edit.php?id=<?php echo (int)$i; ?>#1" method="post">
                        <?php agri_noedit_filter_hiddens($id_ostan1, $id_city, $id_mar, $add_abadi, $add_city, $bah_cod_m, $mor_cod_m, $no_kesh, $z_sal, $mah_qroup, $mah_name, $dis); ?>
                ...
                <li><span class="agri1-pager-ellipsis">...</span></li>
                    <?php } ?>
                <li>
                    <form action="Garden_no_edit.php?id=<?php echo (int)$total; ?>#1" method="post">
                        <?php agri_noedit_filter_hiddens($id_ostan1, $id_city, $id_mar, $add_abadi, $add_city, $bah_cod_m, $mor_cod_m, $no_kesh, $z_sal, $mah_qroup, $mah_name, $dis); ?>
            ...
            <div class="agri1-pager-jump">
                <form id="pageJumpForm" action="Garden_no_edit.php" method="post">
                    <?php agri_noedit_filter_hiddens($id_ostan1, $id_city, $id_mar, $add_abadi, $add_city, $bah_cod_m, $mor_cod_m, $no_kesh, $z_sal, $mah_qroup, $mah_name, $dis); ?>
                    <span>به صفحه</span>
        ...
        (function () {
            ...
            form.addEventListener('submit', function (e) {
                ...
                if (!isNaN(pageId) && pageId >= 1 && pageId <= <?php echo (int)$total; ?>) {
                    this.action = 'Garden_no_edit.php?id=' + pageId + '#1';
                } else {
                    e.preventDefault();
    ...
    <script>
        ...
        (function () {
            ...
            var KEY = 'Garden_no_edit_plot_hidden_cols';
            var GROUP = {
                bah: ['bah_cod_m', 'name'],
                loc: ['abadi', 'city']
            };
