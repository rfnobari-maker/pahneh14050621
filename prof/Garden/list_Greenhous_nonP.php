...
function greenh_nonp_hiddens($y_prod, $id_ostan1, $id_city, $id_mar, $add_abadi, $add_city, $no_mush, $m_ab, $bah_cod_m, $m_cod_m, $mor_cod_m, $no_moj, $no_kesht, $no_saz, $no_gol, $sys_kesh, $sys_hot)
{
...
<body class="agri1-body agri1-page">
    ...
    $pahneh_crumb = array(
        ...
        array('label' => 'لیست واحدهای فاقد عملکرد تولید'),
    );
    include(__DIR__ . '/../../chrome.php');
    ?>
    <main class="agri1-main">
        <header class="agri1-hero">
            <h1 class="agri1-title" id="greenh-nonp-title">لیست واحدهای فاقد عملکرد تولید</h1>
        </header>
        <section class="agri1-card agri1-card-search" aria-labelledby="greenh-nonp-title">
            <form id="reg-form" class="agri1-form" method="post" action="#1" novalidate>
                ...
                <div class="agri1-grid">
                    ...
                    <div class="agri1-field">
                        ...
                        <input name="m_cod_m" type="text" class="input_text" id="m_cod_m" dir="ltr" inputmode="numeric" value="<?php echo agri2_h($m_cod_m); ?>"/>
                    </div>
                    <div class="agri1-field">
                    ...
                    <div class="agri1-field">
                        <label class="agri1-label" for="no_moj">نوع مجوز</label>
                        <select name="no_moj" class="input_text required" id="no_moj">
                            <option value="">انتخاب کنید</option>
                            <?php greenh_opt('1', $no_moj, 'پروانه بهره برداری/نظام مهندسی'); ?>
                            <?php greenh_opt('5', $no_moj, 'پروانه بهره برداری/سازمان جهاد کشاورزی'); ?>
                            <?php greenh_opt('2', $no_moj, 'مشاغل خانگی/وزارت جهاد'); ?>
                            <?php greenh_opt('3', $no_moj, 'تسهیلات/بسیج سازندگی'); ?>
                            <?php greenh_opt('4', $no_moj, 'فاقد مجوز'); ?>
                        </select>
                    </div>
                    <div class="agri1-field" aria-hidden="true"></div>
                    <div class="agri1-field">
                        <label class="agri1-label" for="mor_cod_m">کد ملی کارشناس</label>
                        <input name="mor_cod_m" type="text" class="input_text" id="mor_cod_m" dir="ltr" inputmode="numeric" value="<?php echo agri2_h($login_session); ?>" readonly/>
                    </div>
                </div>
...
if (isset($_POST['action'])) {
    if (!greenh_y_ok($y_prod)) {
        ...
        $where = array('Greenhous_prod.unit_id IS NULL');
        $params = array(':y_prod' => $y_prod);
        $empty0 = array('0', '-1');
        greenh_eq($where, $params, 'Greenhous.id_ostan', $id_ostan1, $empty0);
        greenh_eq($where, $params, 'Greenhous.id_city', $id_city, $empty0);
        greenh_eq($where, $params, 'Greenhous.add_abadi', $add_abadi, $empty0);
        greenh_eq($where, $params, 'Greenhous.add_city', $add_city, $empty0);
        if ($mor_cod_m !== '') {
            $where[] = 'Greenhous.mor_cod_m = :mor_cod_m';
            $params[':mor_cod_m'] = $mor_cod_m;
        }
        if ($bah_cod_m !== '') {
            $where[] = 'Greenhous.bah_cod_m = :bah_cod_m';
            $params[':bah_cod_m'] = $bah_cod_m;
        }
        if ($m_cod_m !== '') {
            $where[] = 'Greenhous.m_cod_m = :m_cod_m';
            $params[':m_cod_m'] = $m_cod_m;
        }
        greenh_eq($where, $params, 'Greenhous.no_mush', $no_mush, $empty0);
        greenh_eq($where, $params, 'Greenhous.no_moj', $no_moj, array(''));
        greenh_eq($where, $params, 'Greenhous.no_kesht', $no_kesht, array(''));
        greenh_eq($where, $params, 'Greenhous.no_gol', $no_gol, array(''));
        greenh_eq($where, $params, 'Greenhous.no_saz', $no_saz, array(''));
        greenh_eq($where, $params, 'Greenhous.sys_kesh', $sys_kesh, array(''));
        greenh_eq($where, $params, 'Greenhous.sys_hot', $sys_hot, array(''));
        $sqlWhere = implode(' AND ', $where);
        $query = "SELECT Greenhous.id, Greenhous.bah_cod_m, Greenhous.num_bah, Greenhous.no_kesht, Greenhous.m_zamin,
                         Greenhous.unit_name, Greenhous.add_abadi, Greenhous.add_city, Greenhous.id_city, Greenhous.id_ostan,
                         Greenhous.no_moj,
                         bah.name, bah.Last_name AS last_name,
                         list_abadi.abadi, list_city.shahr, cityname.city AS city_name, ostanname.ostan
                  FROM Greenhous
                  LEFT JOIN Greenhous_prod ON Greenhous_prod.unit_id = Greenhous.id AND Greenhous_prod.y_prod = :y_prod
                  LEFT JOIN bah ON Greenhous.bah_cod_m = bah.bah_cod_m AND Greenhous.num_bah = bah.num_bah
                  LEFT JOIN list_abadi ON Greenhous.add_abadi <> '' AND Greenhous.add_abadi = list_abadi.add_abadi AND list_abadi.mor_cod_m = Greenhous.mor_cod_m
                  LEFT JOIN list_city ON Greenhous.add_city <> '' AND Greenhous.add_city = list_city.add_city AND list_city.mor_cod_m = Greenhous.mor_cod_m
                  LEFT JOIN cityname ON cityname.id_city = Greenhous.id_city AND cityname.id_ostan = Greenhous.id_ostan
                  LEFT JOIN ostanname ON ostanname.id_ostan = Greenhous.id_ostan
                  WHERE $sqlWhere
...
?>
        <section class="agri1-card agri1-card-results" aria-label="نتایج جستجو">
            <div class="agri1-results-toolbar">
                <div class="agri1-col-picker" id="agri1-col-picker">
                    ...
                    <div id="agri1-col-panel" class="agri1-col-panel" role="group" aria-labelledby="agri1-col-panel-title">
                        ...
                        <ul class="agri1-col-list">
                            ...
                            <li><label><input type="checkbox" data-col-toggle="unit_name" checked/> نام واحد</label></li>
                ...
                <form action="Greenhous_nonP_xls.php" method="post" class="agri1-xls">
                    <?php
                    greenh_hiddens(array(
                        'y_prod' => $y_prod,
                        'id_ostan' => $id_ostan1,
                ...
                <table class="agri1-table">
                    <thead>
                        <tr>
                            ...
                            <th data-col="unit_name" rowspan="2">نام واحد</th>
                    ...
                    ?>
                        <tr>
                            ...
                            <td class="agri1-name-col" data-col="unit_name"><?php echo agri2_h($row['unit_name']); ?></td>
...
<?php
        ...
        <nav class="agri1-pager" aria-label="صفحه‌بندی">
            <ul class="agri1-pager-list">
                <?php if ($id > 1) { ?>
                <li>
                    <form action="list_Greenhous_nonP.php?id=<?php echo (int) $id - 1; ?>#1" method="post">
                        <?php greenh_nonp_hiddens($y_prod, $id_ostan1, $id_city, $id_mar, $add_abadi, $add_city, $no_mush, $m_ab, $bah_cod_m, $m_cod_m, $mor_cod_m, $no_moj, $no_kesht, $no_saz, $no_gol, $sys_kesh, $sys_hot); ?>
                ...
                <?php if ($show_first) { ?>
                <li>
                    <form action="list_Greenhous_nonP.php?id=1#1" method="post">
                        <?php greenh_nonp_hiddens($y_prod, $id_ostan1, $id_city, $id_mar, $add_abadi, $add_city, $no_mush, $m_ab, $bah_cod_m, $m_cod_m, $mor_cod_m, $no_moj, $no_kesht, $no_saz, $no_gol, $sys_kesh, $sys_hot); ?>
                ...
                <li>
                    ...
                    <?php } else { ?>
                    <form action="list_Greenhous_nonP.php?id=<?php echo (int) $i; ?>#1" method="post">
                        <?php greenh_nonp_hiddens($y_prod, $id_ostan1, $id_city, $id_mar, $add_abadi, $add_city, $no_mush, $m_ab, $bah_cod_m, $m_cod_m, $mor_cod_m, $no_moj, $no_kesht, $no_saz, $no_gol, $sys_kesh, $sys_hot); ?>
                ...
                <li><span class="agri1-pager-ellipsis">...</span></li>
                    <?php } ?>
                <li>
                    <form action="list_Greenhous_nonP.php?id=<?php echo (int) $total; ?>#1" method="post">
                        <?php greenh_nonp_hiddens($y_prod, $id_ostan1, $id_city, $id_mar, $add_abadi, $add_city, $no_mush, $m_ab, $bah_cod_m, $m_cod_m, $mor_cod_m, $no_moj, $no_kesht, $no_saz, $no_gol, $sys_kesh, $sys_hot); ?>
            ...
            <div class="agri1-pager-jump">
                <form id="pageJumpForm" action="list_Greenhous_nonP.php" method="post">
                    <?php greenh_nonp_hiddens($y_prod, $id_ostan1, $id_city, $id_mar, $add_abadi, $add_city, $no_mush, $m_ab, $bah_cod_m, $m_cod_m, $mor_cod_m, $no_moj, $no_kesht, $no_saz, $no_gol, $sys_kesh, $sys_hot); ?>
                    <span>به صفحه</span>
        ...
        (function () {
            ...
            form.addEventListener('submit', function (e) {
                ...
                if (!isNaN(pageId) && pageId >= 1 && pageId <= <?php echo (int) $total; ?>) {
                    this.action = 'list_Greenhous_nonP.php?id=' + pageId + '#1';
                } else {
                    e.preventDefault();
    ...
    <script>
        ...
        (function () {
            ...
            var KEY = 'list_Greenhous_nonP_hidden_cols';
            var GROUP = { bah: ['bah_cod_m', 'name'], loc: ['abadi', 'city', 'ostan'] };
