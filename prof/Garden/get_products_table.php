...
// دریافت nah_kesh از جدول Garden
$query_nah = "SELECT nah_kesh FROM Garden WHERE id = :id";
$stmt_nah = $dbh->prepare($query_nah);
$stmt_nah->execute(array(':id' => $id));
$nah_row = $stmt_nah->fetch(PDO::FETCH_ASSOC);
$nah_kesh = $nah_row['nah_kesh'];
...
?>
    <tr class="delete_mem<?php echo $row["id"]?>">
        ...
        <td>
            <select class="mah_kh" id="mah_kh<?php echo $row_index; ?>" style="width:100%; height:38px;" dir="rtl">
        ...
        <td>
            <select class="mah_kh" id="mah_kh<?php echo $row_index; ?>" disabled style="width:100%; height:38px;" dir="rtl">
                <option value="2" selected>خیر</option>
            </select>
        </td>
        <?php endif; ?>
        <td>
            <select class="mah_bem" id="mah_bem<?php echo $row_index; ?>" style="width:100%; height:38px;" dir="rtl">
                <option value="">انتخاب</option>
