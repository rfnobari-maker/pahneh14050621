...
function greenh_rep1_hiddens($id_ostan1, $id_city, $id_mar, $add_abadi, $add_city, $bah_cod_m, $mor_cod_m, $no_mtol, $no_kesht, $y_prod, $m_fani, $v_unit)
{
...
$no_mtol = isset($_POST['no_mtol']) ? $_POST['no_mtol'] : '';
$m_fani = isset($_POST['m_fani']) ? $_POST['m_fani'] : '';
...
<body class="agri1-body agri1-page">
    ...
    $pahneh_crumb = array(
        ...
        array('label' => 'عملکرد سالانه واحد های گلخانه'),
    );
    include(__DIR__ . '/../../chrome.php');
    ?>
    <main class="agri1-main">
        <header class="agri1-hero">
            <h1 class="agri1-title" id="greenh-rep1-title">عملکرد سالانه واحد های گلخانه</h1>
        </header>
        <section class="agri1-card agri1-card-search" aria-labelledby="greenh-rep1-title">
                ...
                <div class="agri1-grid">
                    <div class="agri1-field">
                        ...
                        <select name="y_prod" class="input_text required" id="y_prod">
                            <?php
                            foreach (array('1405','1404', '1403', '1402', '1401', '1400', '1399') as $yy) {
                    ...
                    <div class="agri1-field">
                        <label class="agri1-label" for="seeAnotherField">مسئول فنی</label>
                        <select name="m_fani" class="input_text required" id="seeAnotherField">
                            <option value="">انتخاب کنید</option>
                            <?php greenh_opt('1', $m_fani, 'دارد'); ?>
                            <?php greenh_opt('2', $m_fani, 'ندارد'); ?>
                        </select>
                    </div>
                    <div class="agri1-field">
                        <label class="agri1-label" for="no_mtol">نوع محصول تولیدی</label>
                        <select name="no_mtol" class="input_text required" id="no_mtol">
                            <option value="">انتخاب کنید</option>
                            <?php greenh_opt('211100', $no_mtol, 'سبزی و صیفی'); ?>
                            <?php greenh_opt('211300', $no_mtol, 'گل و گیاه زینتی'); ?>
                            <?php greenh_opt('211200', $no_mtol, 'سایر'); ?>
                        </select>
                    </div>
                    <div class="agri1-field" aria-hidden="true"></div>
                    <div class="agri1-field">
                        <label class="agri1-label" for="v_unit">وضعیت واحد</label>
                        <select name="v_unit" class="input_text required" id="v_unit">
                            <option value="">انتخاب کنید</option>
                            <?php greenh_opt('1', $v_unit, 'فعال'); ?>
                            <?php greenh_opt('2', $v_unit, 'در حال اخذ پروانه تاسیس'); ?>
                            <?php greenh_opt('3', $v_unit, 'دارای پیشرفت فیزیکی'); ?>
                            <?php greenh_opt('4', $v_unit, 'غیرفعال'); ?>
                        </select>
                    </div>
                </div>
...
if (isset($_POST['action'])) {
    if (!greenh_y_ok($y_prod)) {
        ...
        greenh_eq($where, $params, 'P.no_kesht', $no_kesht, array(''));
        ...
        greenh_eq($where, $params, 'P.v_unit', $v_unit, array(''));
        greenh_eq($where, $params, 'P.m_fani', $m_fani, array(''));
        greenh_eq($where, $params, 'P.no_mtol', $no_mtol, array(''));
        $sqlWhere = implode(' AND ', $where);
        $query = "SELECT P.id, P.num_bah, P.bah_cod_m, P.mor_cod_m, P.unit_id, P.t_zan, P.t_mar, P.m_fani,
                         P.no_mtol, P.v_unit, P.id_ostan, P.id_city, P.add_abadi, P.add_city, P.y_prod, P.no_kesht,
                         bah.name, bah.Last_name AS last_name,
                         list_abadi.abadi, list_city.shahr, cityname.city AS city_name, ostanname.ostan
                  FROM Greenhous_prod P
                  LEFT JOIN bah ON P.bah_cod_m = bah.bah_cod_m AND P.num_bah = bah.num_bah
                  LEFT JOIN list_abadi ON P.add_abadi <> '' AND P.add_abadi = list_abadi.add_abadi AND list_abadi.mor_cod_m = P.mor_cod_m
                  LEFT JOIN list_city ON P.add_city <> '' AND P.add_city = list_city.add_city AND list_city.mor_cod_m = P.mor_cod_m
                  LEFT JOIN cityname ON cityname.id_city = P.id_city AND cityname.id_ostan = P.id_ostan
                  LEFT JOIN ostanname ON ostanname.id_ostan = P.id_ostan
                  WHERE $sqlWhere
                  ORDER BY P.bah_cod_m ASC
                  LIMIT $start, $limit";
        $query1 = "SELECT COUNT(*) FROM Greenhous_prod P WHERE $sqlWhere";
...
?>
        <section class="agri1-card agri1-card-results" aria-label="نتایج جستجو">
            <div class="agri1-results-toolbar">
                <div class="agri1-col-picker" id="agri1-col-picker">
                    ...
                    <div id="agri1-col-panel" class="agri1-col-panel" role="group" aria-labelledby="agri1-col-panel-title">
                        ...
                        <ul class="agri1-col-list">
                            ...
                            <li><label><input type="checkbox" data-col-toggle="workers" checked/> تعداد شاغل</label></li>
                            <li><label><input type="checkbox" data-col-toggle="m_fani" checked/> مسئول فنی</label></li>
                            <li><label><input type="checkbox" data-col-toggle="no_mtol" checked/> نوع محصول تولیدی</label></li>
                            <li><label><input type="checkbox" data-col-toggle="no_kesht" checked/> نوع کشت</label></li>
                            <li><label><input type="checkbox" data-col-toggle="v_unit" checked/> وضعیت واحد</label></li>
                            <li><label><input type="checkbox" data-col-toggle="y_prod" checked/> عملکرد سال</label></li>
                ...
                <form action="Greenh_prod_xls.php" method="post" class="agri1-xls">
                    <?php
                    greenh_hiddens(array(
                        'id_ostan' => $id_ostan1,
                        'id_city' => $id_city,
                ...
                <table class="agri1-table">
                    <thead>
                        <tr>
                            ...
                            <th data-col="workers" rowspan="2">تعداد شاغل<br/>نفر</th>
                            <th data-col="m_fani" rowspan="2">مسئول فنی</th>
                            <th data-col="no_mtol" rowspan="2">نوع محصول تولیدی</th>
                            <th data-col="no_kesht" rowspan="2">نوع کشت</th>
                            <th data-col="v_unit" rowspan="2">وضعیت واحد</th>
                            <th data-col="y_prod" rowspan="2">عملکرد سال</th>
                            ...
                            <td class="agri1-ops" data-col="ops">
                                <div class="agri1-ops-bar">
                                    <form action="Greenh_prod_view.php" method="post" onsubmit="target_po3(this)">
                                        ...
                                        <input type="hidden" name="v_unit" value="<?php echo agri2_h($row['v_unit']); ?>"/>
                                        <input type="hidden" name="num_bah" value="<?php echo agri2_h($row['num_bah']); ?>"/>
                                        <input type="hidden" name="no_moj" value=""/>
                                        <button type="submit" class="agri1-ops-btn" title="نمایش اطلاعات عملکرد واحد" aria-label="نمایش اطلاعات عملکرد واحد"><?php echo greenh_ops_svg('view'); ?></button>
                                    </form>
                                </div>
                            </td>
                            <td data-col="workers"><?php echo agri2_h(($row['t_mar'] + $row['t_zan']) * 1); ?></td>
                            <td data-col="m_fani"><?php echo agri2_h(greenh_label_m_fani($row['m_fani'])); ?></td>
                            <td data-col="no_mtol"><?php echo agri2_h(greenh_label_no_mtol($row['no_mtol'])); ?></td>
                            <td data-col="no_kesht"><?php echo agri2_h(greenh_label_no_kesht($row['no_kesht'])); ?></td>
                            <td data-col="v_unit"><?php echo agri2_h(greenh_label_v_unit($row['v_unit'])); ?></td>
                            <td data-col="y_prod"><?php echo agri2_h($row['y_prod']); ?></td>
...
<?php
        ...
        <nav class="agri1-pager" aria-label="صفحه‌بندی">
            <ul class="agri1-pager-list">
                <?php if ($id > 1) { ?>
                <li>
                    <form action="Greenh_rep1.php?id=<?php echo (int) $id - 1; ?>#1" method="post">
                        <?php greenh_rep1_hiddens($id_ostan1, $id_city, $id_mar, $add_abadi, $add_city, $bah_cod_m, $mor_cod_m, $no_mtol, $no_kesht, $y_prod, $m_fani, $v_unit); ?>
                ...
                <?php if ($show_first) { ?>
                <li>
                    <form action="Greenh_rep1.php?id=1#1" method="post">
                        <?php greenh_rep1_hiddens($id_ostan1, $id_city, $id_mar, $add_abadi, $add_city, $bah_cod_m, $mor_cod_m, $no_mtol, $no_kesht, $y_prod, $m_fani, $v_unit); ?>
                ...
                <li>
                    ...
                    <?php } else { ?>
                    <form action="Greenh_rep1.php?id=<?php echo (int) $i; ?>#1" method="post">
                        <?php greenh_rep1_hiddens($id_ostan1, $id_city, $id_mar, $add_abadi, $add_city, $bah_cod_m, $mor_cod_m, $no_mtol, $no_kesht, $y_prod, $m_fani, $v_unit); ?>
                ...
                <li><span class="agri1-pager-ellipsis">...</span></li>
                    <?php } ?>
                <li>
                    <form action="Greenh_rep1.php?id=<?php echo (int) $total; ?>#1" method="post">
                        <?php greenh_rep1_hiddens($id_ostan1, $id_city, $id_mar, $add_abadi, $add_city, $bah_cod_m, $mor_cod_m, $no_mtol, $no_kesht, $y_prod, $m_fani, $v_unit); ?>
            ...
            <div class="agri1-pager-jump">
                <form id="pageJumpForm" action="Greenh_rep1.php" method="post">
                    <?php greenh_rep1_hiddens($id_ostan1, $id_city, $id_mar, $add_abadi, $add_city, $bah_cod_m, $mor_cod_m, $no_mtol, $no_kesht, $y_prod, $m_fani, $v_unit); ?>
                    <span>به صفحه</span>
        ...
        (function () {
            ...
            form.addEventListener('submit', function (e) {
                ...
                if (!isNaN(pageId) && pageId >= 1 && pageId <= <?php echo (int) $total; ?>) {
                    this.action = 'Greenh_rep1.php?id=' + pageId + '#1';
                } else {
                    e.preventDefault();
    ...
    <script>
        ...
        (function () {
            ...
            var KEY = 'Greenh_rep1_hidden_cols';
