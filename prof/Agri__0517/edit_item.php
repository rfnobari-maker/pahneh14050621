...
if (isset($_POST["id"])) {
    ...
    try {
        ...
        $stmt_old = $dbh->prepare("SELECT cod_qroup, cod_mah, zer_kesht_a, zer_kesht_b FROM `$Agri_prod_table` WHERE id=?");
        ...
        if (!isset($id_ostan) || !isset($id_city) || !isset($id_mar)) {
            echo 'خطا: اطلاعات مکان (استان، شهرستان ، مرکز) یافت نشد.';
            if (isset($_SESSION)) session_destroy();
            if (isset($dbh)) $dbh = null;
            $actual_link = "https://" . $_SERVER['HTTP_HOST'];
            header("Location: {$actual_link}/login/login.php?message=missing_location_data");
            exit();
        }
