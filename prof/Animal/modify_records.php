...
$query = "UPDATE Mushroom_spawn SET num_row=?, num_t_row=? , h_t=? , w_t=? , zer_kesh=? WHERE id=?";
$q = $dbh->prepare($query);
...
{
$query = "INSERT INTO Mushroom_spawn (unit_id,y_prod,num_row,num_t_row,h_t,w_t,zer_kesh)
 VALUES (:unit_id,:y_prod,:num_row,:num_t_row,:h_t,:w_t,:zer_kesh)";
$q = $dbh->prepare($query);
$q->execute(array(':unit_id'=>$unit_id,':y_prod'=>$y_prod,':num_row'=>$num_row,':num_t_row'=>$num_t_row,':h_t'=>$h_t,':w_t'=>$w_t,':zer_kesh'=>$zer_kesh));
