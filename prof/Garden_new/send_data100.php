...
$query ="CREATE TEMPORARY TABLE Garden_p$id SELECT * FROM Garden_prod WHERE Garden_id=:Garden_id " ;
$stmt = $dbh->prepare($query);
$stmt->execute(array(':Garden_id'=>$id));

$query = "update Garden_p$id set date_s = :date_s ,id = '',Garden_id = '', z_sal='1396' , mah_tol=0 , mah_tolp=0  where bah_cod_m = :bah_cod_m and mor_cod_m = :mor_cod_m ";
$stmt = $dbh->prepare($query);
$stmt->execute(array(':date_s'=>$date_edit,':bah_cod_m'=>$bah_cod_m,':mor_cod_m'=>$mor_cod_m));
$query = "INSERT INTO Garden_prod SELECT * FROM Garden_p$id" ;  	
$stmt = $dbh->prepare($query);
$stmt->execute();
...
$query = "update Garden$id set date_s = :date_s ,id = '',z_sal='1396' where bah_cod_m = :bah_cod_m and mor_cod_m = :mor_cod_m ";
$stmt = $dbh->prepare($query);
$stmt->execute(array(':date_s'=>$date_edit,':bah_cod_m'=>$bah_cod_m,':mor_cod_m'=>$mor_cod_m));
...
//alert($bah_cod_m) ;
//alert($mor_cod_m) ;
$query = "SELECT id,sh_gat from Garden where bah_cod_m=:bah_cod_m and z_sal = :z_sal and mor_cod_m = :mor_cod_m  "; 
$stmt = $dbh->prepare($query);
$stmt->execute(array(':bah_cod_m'=>$bah_cod_m,':z_sal'=>'1396',':mor_cod_m'=>$mor_cod_m));
foreach($stmt as $row){
$id = $row['id'] ;
$sh_gat = $row['sh_gat'] ;
//alert($sh_gat) ;
$query = "UPDATE Garden_prod SET Garden_id=? WHERE bah_cod_m=? and sh_gat =? and z_sal='1396' and mor_cod_m=? and Garden_id = 0 ";
$q = $dbh->prepare($query);
$q->execute(array($id,$bah_cod_m,$sh_gat,$mor_cod_m));
}
//
