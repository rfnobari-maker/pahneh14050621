...
$stmt = $dbh->prepare($query);
$stmt->execute(array(':bah_cod_m'=>$bah_cod_m,':z_sal'=>'1395',':mor_cod_m'=>$mor_cod_m,':Garden_id'=>$id));
$query = "update tmp$id set date_s = :date_s ,id = '',Garden_id = '', z_sal='1396' , mah_tol=0 , mah_tolp=0  where bah_cod_m = :bah_cod_m and mor_cod_m = :mor_cod_m ";
$stmt = $dbh->prepare($query);
...
//alert($sh_gat) ;
$query = "UPDATE Garden_prod SET Garden_id=? WHERE bah_cod_m=? and sh_gat =? and check_cod = ? and z_sal='1396' and mor_cod_m=? and Garden_id = 0 ";
$q = $dbh->prepare($query);
