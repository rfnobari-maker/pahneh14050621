...
$URL = 'http://10.7.234.126/web/Wheat97_WS.php' ; 
$namespace = $URL . '?wsdl';
//using soap_server to create server object
$server    = new soap_server;
function getProd($username,$password,$id_ostan,$id_city,$no_bah,$meli) {
...
GROUP BY Agri_prod.bah_cod_m,Agri_prod.`num_bah`"; 
$stmt = $dbh->prepare($query);
$stmt->execute();
//$row = $stmt->fetchAll();
$row = $stmt->fetch(PDO::FETCH_ASSOC);
if ($no_bah == '2') {  $row['bah_cod_m'] = $row['sh_meli'] ;  $row['name'] = '' ;  $row['last_name'] = '' ;  $row['fname'] = '' ;  }
return $row;
	}
	else {
            return "error";
...
$server->configureWSDL('WHeat', $namespace);
