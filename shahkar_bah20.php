...
function processRecords($limit) {
    ...
    $stmt = $dbh->prepare("SELECT id, bah_cod_m AS national_code, tel_m AS mobile_number FROM bah WHERE valid not in ('200','600')  and  ok = '1' LIMIT :limit");
    ...
    foreach ($records as $record) {
        ...
        if ($response && isset($response['result']['data']['response'])) {
            ...
            $updateStmt = $dbh->prepare("UPDATE bah SET valid = :shahkarStatus , date_s = '1404/10/09' WHERE id = :id");
...
$limit = 100; // تعداد رکوردها برای بررسی
