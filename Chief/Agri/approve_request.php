...
// بررسی فیلدهای ضروری
if (empty($data['username']) || empty($data['id_ostan']) || empty($data['z_sal'])) {
    $response = array(
        'status' => 'error',
        'message' => 'نام کاربری، کد استان یا سال زراعی وجود ندارد'
    );
    echo json_encode($response);
    exit;
}
...
try {
    ...
    $updateStmt = $dbh->prepare("UPDATE $Agri_prod_table SET app_level = :app_level, app_date = :app_date WHERE id_ostan = :id_ostan and app_level = '3'");
