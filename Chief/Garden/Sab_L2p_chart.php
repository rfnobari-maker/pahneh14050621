...
<div class="container">
    ...
    if (!empty($mah_qroup) && !empty($mah_name) && !empty($z_sal) && !empty($id_ostan1)) {
        ...
        if ($stmt && $stmt->rowCount() > 0) {
            $labels = array();
            $city_ids = array(); // برای ذخیره id_city
            $data = array(
            ...
            while ($r = $stmt->fetch(PDO::FETCH_ASSOC)) {
                $labels[] = $r['city'];
                $city_ids[] = $r['id_city']; // ذخیره id_city
    ...
    <script>
        ...
        var cityIds = <?php echo json_encode($city_ids); ?>; // آرایه id_city
                ...
                options: {
                    ...
                    onClick: function(e, el) {
                        if (el.length > 0) {
                            ...
                            form.method = 'POST';
                            form.action = 'Sab_L3p_chart.php';
                            form.target = 'Sab_L3_popup';
                            ...
                            window.open('about:blank', 'Sab_L3_popup', 'width=1100,height=800');
