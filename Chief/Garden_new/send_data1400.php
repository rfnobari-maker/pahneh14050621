...
$stmt->execute(array(':bah_cod_m'=>$bah_cod_m,':z_sal'=>'1400',':mor_cod_m'=>$login_session));
$found = $stmt -> rowCount();
    if ($found > 0) {
         $com_alert = 'خطا : \n \n برای بهره بردار قبلا اطلاعات  1400  ثبت شده است' ;
    }
else 
{
foreach($idArr as $id){
...
$stmt = $dbh->prepare($query);
$stmt->execute(array(':bah_cod_m'=>$bah_cod_m,':z_sal'=>'1398',':mor_cod_m'=>$mor_cod_m,':Garden_id'=>$id));
$query = "update tmp$id set date_s = :date_s ,id = '',Garden_id = '', z_sal='1400' , mah_tol=0 , mah_tolp=0  where bah_cod_m = :bah_cod_m and mor_cod_m = :mor_cod_m ";
$stmt = $dbh->prepare($query);
...
$stmt = $dbh->prepare($query);
$stmt->execute(array(':bah_cod_m'=>$bah_cod_m,':z_sal'=>'1398',':mor_cod_m'=>$mor_cod_m,':id'=>$id));
$query = "update Garden$id set date_s = :date_s ,id = '',z_sal='1400' where bah_cod_m = :bah_cod_m and mor_cod_m = :mor_cod_m ";
$stmt = $dbh->prepare($query);
$stmt->execute(array(':date_s'=>$date_edit,':bah_cod_m'=>$bah_cod_m,':mor_cod_m'=>$mor_cod_m));
...
$stmt = $dbh->prepare($query);
$stmt->execute(array(':bah_cod_m'=>$bah_cod_m,':z_sal'=>'1400',':mor_cod_m'=>$mor_cod_m));
foreach($stmt as $row){
$id = $row['id'] ;
$sh_gat = $row['sh_gat'] ;
$check_cod = $row['check_cod'] ;
//alert($sh_gat) ;
$query = "UPDATE Garden_prod SET Garden_id=? WHERE bah_cod_m=? and sh_gat =? and check_cod = ? and z_sal='1400' and mor_cod_m=? and Garden_id = 0 ";
$q = $dbh->prepare($query);
$q->execute(array($id,$bah_cod_m,$sh_gat,$check_cod,$mor_cod_m));
	     ...
	     $com_alert = 'اطلاعات با موفقیت ارسال شد.   \n \n اکنون می توانید نسبت به ویرایش اطلاعات 1400  اقدام فرمایید ';
 }
 }
 ...
 ?>
<form name="myform1" class="myform" method="post" action="New_Garden1400.php">
