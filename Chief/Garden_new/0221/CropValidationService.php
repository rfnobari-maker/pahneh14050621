...
class CropValidationService {
     ...
     * @param float $current_batch_total مجموع سطح کشت در بچ جاری
    ...
    public function validateCultivatedAreaAgainstAllocation(
        ...
        // محاسبه مجموع نهایی (قبلی + بچ جاری + جدید)
        $overall_total_bar = $current_total_bar + $current_batch_total + $s_kesht_b_new;
        $overall_total_nobar = $current_total_nobar + $current_batch_total + $s_kesht_gb_new;
     ...
     *
     * @param string $product_code کد محصول
     * @param float $cultivated_area سطح کشت (هکتار) - مجموع بارور و غیربارور
    ...
    {
        // اگر سطح کشت صفر است، تولید هم باید صفر باشد
        if ($cultivated_area <= 0 && $production_amount > 0) {
            $type_text = ($production_type == 'pishbini') ? 'پیش‌بینی' : 'قطعی';
            return array(
                'isValid' => false,
                'message' => "امکان ثبت میزان تولید $type_text بدون ثبت سطح کشت وجود ندارد"
            );
        }
