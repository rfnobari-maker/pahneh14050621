...
// جمع کل سطح کشت فعلی (بارور + غیربارور) برای این باغ
...
// محاسبه مجموع سطح کشت جدید از آرایه‌های دریافتی
$new_rows_total = 0;
...
if ($overall_total > $m_zamin_server) {
    ...
    foreach ($cod_qroup as $value) {
        ...
        // برای نحوه کشت "درختان پراکنده" (nah_kesh=3) سطح کشت نباید ثبت شود
        if ($nah_kesh == '3') {
...
// ========== اضافه کن ==========
// بررسی: اگر همه مقادیر سطح کشت و تعداد درخت صفر هستند، ثبت ممنوع باشد
$s_kesht_b_val = (float)$s_kesht_b[$count];
$s_kesht_gb_val = (float)$s_kesht_gb[$count];
$tree_b_val = (int)$tree_b[$count];
$tree_gb_val = (int)$tree_gb[$count];

if ($s_kesht_b_val == 0 && $s_kesht_gb_val == 0 && $tree_b_val == 0 && $tree_gb_val == 0) {
...
// ========== پایان اضافه ==========
            ...
            // دریافت مجموع تجمعی برای این محصول در بچ جاری
...
// محاسبه سطح کشت کل برای اعتبارسنجی تولید
$cultivated_area_total = (float)$s_kesht_b[$count] + (float)$s_kesht_gb[$count];
...
if (!$validationResult['isValid'] || !$tolp_validation['isValid'] || !$tol_validation['isValid']) {
    ...
    continue;
            ...
            if (true) {
                ...
                // به‌روزرسانی مجموع تجمعی برای بچ جاری
                $batch_cumulative_sums[$key_for_sum] = $current_batch_total + $current_cultivated_area_bar + $current_cultivated_area_nobar;
