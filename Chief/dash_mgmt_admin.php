<?php
/**
 * Dashboard admin — simplified, no side_menu (avoids white screen).
 * TEMP: any logged-in user. Tighten later after move.
 */
if (session_id() === '') {
    session_start();
}
...
if ($user_check === '' || empty($dbh)) {
    header('Location: ' . $actual_link . '/login/login.php');
    exit;
}

$stmt = $dbh->prepare(
    "SELECT username, Last_name, name, S_access FROM users WHERE username = ? LIMIT 1"
);
if (!$stmt) {
...
$PersName = $userRow['Last_name'];
$_SESSION['last_acted_on'] = time();

$lib = dirname(__FILE__) . '/dash_mgmt_lib.php';
if (!is_file($lib)) {
    die('missing dash_mgmt_lib.php');
}
require_once $lib;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'set_status') {
        ...
        header('Location: dash_mgmt_admin.php');
        exit;
    }
...
<div class="dash-admin">
    ...
    <p class="sub">کاربر: <?php echo dash_admin_h($userRow['username']); ?> — موقتاً بدون قفل دسترسی</p>

    <div class="dash-admin-actions">
        <a class="dash-admin-btn primary" href="dash_mgmt.php">بازگشت به داشبورد</a>
    ...
    <section class="dash-admin-card">
        ...
        <p class="dash-admin-note">
            Cron:<br>
            <code>0 4 * * * /usr/bin/php /path/to/Chief/dash_snap_cron.php &gt;&gt; /var/log/dash_snap_cron.log 2&gt;&amp;1</code>
        </p>
    </section>
</div>
</body>
