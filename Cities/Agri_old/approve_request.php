...
include('../../lock_p3.php'); // اینجا متغیرهای id_ostan, id_city, id_mar را دریافت می‌کنیم
...
// بررسی فیلدهای ضروری
if (empty($data['username']) || empty($data['id_mar']) || empty($data['z_sal'])) {
    $response = array(
        'status' => 'error',
        'message' => 'نام کاربری، کد مرکز یا سال زراعی وجود ندارد'
    );
    echo json_encode($response);
    exit;
}
...
try {
    ...
    $updateStmt = $dbh->prepare("UPDATE $Agri_prod_table SET app_level = :app_level, app_date = :app_date WHERE id_city = :id_city and app_level in ('1','20')");
