...
?>
   ...
   <?php if ($row['group_cod']==$mah_qroup) echo 'selected=selected'?>> <?php echo $row['group_name'] ;?></option>
        ...
        <td height="55"  align='center' bgcolor="#DDDDDD" class="style11"><font size="2" class="style8">: انتخاب گروه</font></td>
      </tr>
      <tr >
...
?>
        ...
        <td height="50"  align='center' bgcolor="#FFFFFF" class="style11"><font size="2" class="style8">: نام محصول</font></td>
      </tr>
      <tr >
        ...
        <td height="50" align="left" class="style8"> :کد ملی بهره بردار </td>
      </tr>
      <tr >
...
<?php 
 ...
 ?>
           ...
           <br />
           <table width="100%"  align="center" class="my-table" >
...
inner join users On del_rec.mor_cod_m = users.username
 WHERE del_rec.Table_name = '$Agri_prod_table' and $v_cod_mah and users.id_ostan = '$id_ostan' and users.id_city = '$id_city' and $v_mor_cod_m and $v_bah_cod_m ORDER BY del_rec.Date DESC LIMIT $start, $limit " ;
  $query1 = "SELECT count(*)
FROM del_rec
inner join users On del_rec.mor_cod_m = users.username
 WHERE del_rec.Table_name = '$Agri_prod_table' and $v_cod_mah and users.id_ostan = '$id_ostan' and users.id_city = '$id_city' and $v_mor_cod_m and $v_bah_cod_m " ;
$stmt = $dbh->prepare($query);
$stmt->execute();
$r = $start+1 ;
 foreach($stmt as $row){
