...
// مقادیر اولیه  محصول 490 برای اعتبارسنجی سمت سرور
...
// دریافت مقادیر نهایی تراز از فرم
$final_s_abi = clean_number($_POST['final_s_abi']);
$final_s_dem = clean_number($_POST['final_s_dem']);
$final_t_abi = clean_number($_POST['final_t_abi']);
$final_t_dem = clean_number($_POST['final_t_dem']);
...
// به‌روزرسانی مقدار محصول 490 با مقادیر اولیه
// از concat برای قرار دادن مستقیم متغیرها در کوئری استفاده کنید
//$query_update_490 = "UPDATE Agri_ab_mar SET s_abi= $initial_s_abi, s_dem= $initial_s_dem, t_abi=$initial_t_abi, t_dem = $initial_t_dem WHERE id_mar ='$id_mar' AND product_cod = '490' AND z_sal ='$z_sal'";
//$stmt_update_490 = $dbh->prepare($query_update_490);
//$stmt_update_490->execute(array());
...
// بررسی اینکه آیا به روزرسانی موفقیت‌آمیز بوده است
 ...
 //   }
    ...
    // فقط در صورت وجود داده برای ثبت، عملیات حذف و ثبت را انجام می‌دهد.
        ...
        $group_cod = '4' ; 
		$query_delete = "DELETE FROM Agri_ab_mar WHERE id_mar = ? AND z_sal = ? and group_cod = ? AND product_cod not in ('490','170','172','174')";
