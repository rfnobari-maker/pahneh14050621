...
$query ="UPDATE list_abadi SET mor_cod_m=?	WHERE  mor_cod_m=? and add_abadi=?";
$q = $dbh->prepare($query);
$q->execute(array($cod_m2,$cod_m1,$add_abadi));
edit_database_abadi($id_mar,$cod_m2,$id_abadi,$add_abadi) ;
$query = "UPDATE change_mor SET status=? , date_a = ? WHERE mor_codm_old=? and mor_codm_new=? and add_abadi = ? ";
$q = $dbh->prepare($query);
