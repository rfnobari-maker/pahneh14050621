<?php
/**
 * نوار بالا + منو + کارت کاربر — نسخهٔ مشترک ریشهٔ سایت
 * مسیر لینک‌ها از روی محل صفحهٔ جاری حساب می‌شود (ریشه، prof، Agri، Garden، …).
...
if (!function_exists('pahneh_chrome_theme')) {
    function pahneh_chrome_theme()
    {
        static $theme = null;
        if ($theme !== null) {
            return $theme;
        }
        $allowed = array('farm', 'calm', 'paper', 'contrast');
        $t = isset($_COOKIE['pahneh_theme']) ? $_COOKIE['pahneh_theme'] : 'farm';
        $theme = in_array($t, $allowed, true) ? $t : 'farm';
        return $theme;
    }
}
...
if (!function_exists('pahneh_chrome_crumbs')) {
    function pahneh_chrome_crumbs($toRoot, $toProf)
    {
        ...
        $pages = array(
            ...
            'list_expar_sh.php' => 'کارشناسان موضوعی',
            'list_scholar.php' => 'محقق معین شهرستان',
            'list_admin.php' => 'ادمین استانی سامانه',
            'area_experts.php' => 'ارتباط با سایر کارشناسان پهنه',
...
$pahneh_site_dir = __DIR__;
$pahneh_prof_dir = __DIR__ . DIRECTORY_SEPARATOR . 'prof';
...
if ($markaz !== '') {
    $place_primary = $markaz;
    $place_meta_city = $city;
    $place_meta_ostan = $ostan;
} elseif ($city !== '') {
    $place_primary = $city;
    $place_meta_city = '';
    $place_meta_ostan = $ostan;
...
<link href="<?php echo pahneh_chrome_h($toRoot); ?>agri1-theme.css" rel="stylesheet" type="text/css"/>
<script>
document.documentElement.setAttribute('data-theme', <?php echo json_encode(pahneh_chrome_theme()); ?>);
</script>
<style>
    .agri1-chrome {
        --space-1: 8px;
    ...
    .agri1-chrome-brand-short { display: none; }
    ...
    .agri1-chrome-place-primary {
        flex: 0 0 auto;
        font-weight: 600;
    }
    .agri1-chrome-place-meta {
        display: inline-flex;
        align-items: center;
        min-width: 0;
        overflow: hidden;
    ...
    .agri1-chrome-place-part,
    .agri1-chrome-place-sep {
        flex: 0 0 auto;
    }
    .agri1-chrome-place-chip.is-hide-ostan [data-place="ostan"],
    .agri1-chrome-place-chip.is-hide-ostan [data-place-sep="ostan"],
    .agri1-chrome-place-chip.is-hide-city [data-place="city"],
    .agri1-chrome-place-chip.is-hide-city [data-place-sep="city"],
    .agri1-chrome-place-chip.is-hide-ostan.is-hide-city .agri1-chrome-place-meta {
    ...
    .agri1-chrome-theme-list {
        display: grid;
        grid-template-columns: 1fr 1fr;
        gap: 6px;
    }
    .agri1-chrome-theme-btn {
        display: flex;
        align-items: center;
        gap: 8px;
        min-height: var(--touch);
    ...
    .agri1-chrome-theme-btn:hover {
        background: var(--color-muted);
    }
    .agri1-chrome-theme-btn[aria-pressed="true"] {
        border-color: var(--color-primary);
        background: var(--color-background);
        color: var(--color-primary);
    }
    .agri1-chrome-theme-swatch {
        width: 16px;
        height: 16px;
        flex: 0 0 auto;
        border-radius: 50%;
    ...
    @media (max-width: 1100px) {
        .agri1-chrome-place-part[data-place="ostan"],
        .agri1-chrome-place-sep[data-place-sep="ostan"] {
            display: none !important;
        }
    }
    ...
    @media (max-width: 900px) {
        ...
        .agri1-chrome-brand { max-width: min(340px, 42vw); }
        .agri1-chrome-brand-title { font-size: 0.875rem; }
        .agri1-chrome-brand-sub { font-size: 0.6875rem; }
        .agri1-chrome-brand-mark { width: 53px; height: 53px; }
    }

    @media (max-width: 820px) {
        .agri1-chrome-place-part[data-place="city"],
        .agri1-chrome-place-sep[data-place-sep="city"],
        .agri1-chrome-place-meta {
            display: none !important;
        }
    }
    ...
    @media (max-width: 640px) {
        ...
        .agri1-chrome-account-name { max-width: 88px; }
        .agri1-chrome-tools {
            flex: 1 1 auto;
        ...
        .agri1-chrome-brand-mark { width: 40px; height: 40px; }
        .agri1-chrome-brand-text {
            margin: 0;
            align-items: center;
        }
        .agri1-chrome-brand-title,
        .agri1-chrome-brand-sub { display: none; }
        .agri1-chrome-brand-short {
            display: block;
            max-width: 100%;
            overflow: hidden;
            text-overflow: ellipsis;
    ...
    @media (max-width: 480px) {
        .agri1-chrome-account-name { display: none; }
        .agri1-chrome-account-btn { padding-inline-end: 4px; }
    }
    ...
    <div class="agri1-chrome-bar">
        <div class="agri1-chrome-brand">
            <span class="agri1-chrome-brand-mark">
                <img src="<?php echo pahneh_chrome_h($toProf); ?>1.png" width="59" height="59" alt="جهاد کشاورزی"/>
            </span>
            <p class="agri1-chrome-brand-text" title="سامانه جامع پهنه بندی و مدیریت داده های کشاورزی">
                <span class="agri1-chrome-brand-title">سامانه جامع پهنه بندی و مدیریت داده های کشاورزی</span>
                <span class="agri1-chrome-brand-sub">کارتابل ویژه کارشناس پهنه</span>
                <span class="agri1-chrome-brand-short">پهنه بندی</span>
            </p>
        </div>
        <div class="agri1-chrome-tools">
            <?php if ($place_text !== '') { ?>
                <span class="agri1-chrome-chip agri1-chrome-place-chip" title="<?php echo pahneh_chrome_h($place_text); ?>">
                    <?php echo pahneh_chrome_icon('pin'); ?>
                    <span class="agri1-chrome-sr"><?php echo pahneh_chrome_h($place_text); ?></span>
                    <span class="agri1-chrome-place-text" aria-hidden="true">
                        <span class="agri1-chrome-place-primary"><?php echo pahneh_chrome_h($place_primary); ?></span>
                        <?php if ($place_meta_city !== '' || $place_meta_ostan !== '') { ?>
                            <span class="agri1-chrome-place-meta">
                                <span class="agri1-chrome-place-sep"> · </span>
                                <?php if ($place_meta_city !== '') { ?>
                                    <span class="agri1-chrome-place-part" data-place="city"><?php echo pahneh_chrome_h($place_meta_city); ?></span>
                                <?php } ?>
                                <?php if ($place_meta_city !== '' && $place_meta_ostan !== '') { ?>
                                    <span class="agri1-chrome-place-sep" data-place-sep="ostan"> · </span>
                                <?php } ?>
                                <?php if ($place_meta_ostan !== '') { ?>
                                    <span class="agri1-chrome-place-part" data-place="ostan"><?php echo pahneh_chrome_h($place_meta_ostan); ?></span>
                                <?php } ?>
                            </span>
                        <?php } ?>
                    </span>
            ...
            <div class="agri1-chrome-account">
                ...
                <div class="agri1-chrome-account-menu" id="agri1-chrome-account-menu">
                    ...
                    <?php
                    $pahneh_theme_now = pahneh_chrome_theme();
                    $pahneh_theme_opts = array(
                        'farm' => array('label' => 'نعنایی', 'swatch' => '#86C9A0'),
                        'calm' => array('label' => 'آرام', 'swatch' => '#F7F8F5'),
                        'paper' => array('label' => 'کاغذی', 'swatch' => '#F7F3EB'),
                        'contrast' => array('label' => 'پررنگ', 'swatch' => '#FFFFFF'),
                    );
                    ?>
                    <div class="agri1-chrome-theme" role="group" aria-label="رنگ صفحه">
                        <span class="agri1-chrome-theme-label" id="agri1-chrome-theme-label">رنگ صفحه</span>
                        <div class="agri1-chrome-theme-list">
                            <?php foreach ($pahneh_theme_opts as $tid => $opt) { ?>
                                <button type="button" class="agri1-chrome-theme-btn" data-theme-set="<?php echo pahneh_chrome_h($tid); ?>" aria-pressed="<?php echo $pahneh_theme_now === $tid ? 'true' : 'false'; ?>">
                                    <span class="agri1-chrome-theme-swatch" style="background:<?php echo pahneh_chrome_h($opt['swatch']); ?>" aria-hidden="true"></span>
                                    <span><?php echo pahneh_chrome_h($opt['label']); ?></span>
                                </button>
                            <?php } ?>
                        </div>
                    </div>
    ...
    <nav class="agri1-chrome-nav" aria-label="منوی سامانه">
        ...
        <ul id="agri1-chrome-menu" class="agri1-chrome-menu">
            ...
            <li class="agri1-chrome-item is-parent">
                ...
                <ul class="agri1-chrome-sub">
                    <li><a href="<?php echo pahneh_chrome_h($toRoot); ?>search_promo.php">جستجوی کاربر</a></li>
                    <li><a href="<?php echo pahneh_chrome_h($toRoot); ?>list_expar.php">کارشناسان معین</a></li>
                    <li><a href="<?php echo pahneh_chrome_h($toRoot); ?>list_expar_sh.php">کارشناسان موضوعی</a></li>
                    <li><a href="<?php echo pahneh_chrome_h($toRoot); ?>list_scholar.php">محقق معین شهرستان</a></li>
                    <li><a href="<?php echo pahneh_chrome_h($toRoot); ?>list_Admin.php">ادمین استانی سامانه</a></li>
                    <li><a href="<?php echo pahneh_chrome_h($toRoot); ?>Area_experts.php">ارتباط با سایر کارشناسان پهنه</a></li>
                </ul>
            </li>
            <li class="agri1-chrome-item is-parent">
                <div class="agri1-chrome-row">
...
(function () {
    ...
    if (account && accountBtn) {
        ...
        function pahnehApplyTheme(id) {
            var allowed = { farm: 1, calm: 1, paper: 1, contrast: 1 };
            if (!allowed[id]) id = 'farm';
            document.documentElement.setAttribute('data-theme', id);
            document.cookie = 'pahneh_theme=' + encodeURIComponent(id) + ';path=/;max-age=31536000;SameSite=Lax';
            var btns = account.querySelectorAll('[data-theme-set]');
            for (var i = 0; i < btns.length; i++) {
                btns[i].setAttribute('aria-pressed', btns[i].getAttribute('data-theme-set') === id ? 'true' : 'false');
            }
        }
        account.addEventListener('click', function (e) {
            var btn = e.target.closest('[data-theme-set]');
            if (!btn || !account.contains(btn)) return;
            e.preventDefault();
            e.stopPropagation();
            pahnehApplyTheme(btn.getAttribute('data-theme-set'));
        });
    }
    ...
    var placeChip = root.querySelector('.agri1-chrome-place-chip');
    var chromeBar = root.querySelector('.agri1-chrome-bar');
    var chromeTools = root.querySelector('.agri1-chrome-tools');
    var placeFitRaf = 0;
    function placeNaturalWidth() {
        if (!placeChip) return 0;
        var prevWidth = placeChip.style.width;
        var prevMax = placeChip.style.maxWidth;
        var prevFlex = placeChip.style.flex;
        placeChip.style.width = 'max-content';
        placeChip.style.maxWidth = 'none';
        placeChip.style.flex = '0 0 auto';
        var w = placeChip.scrollWidth;
        placeChip.style.width = prevWidth;
        placeChip.style.maxWidth = prevMax;
        placeChip.style.flex = prevFlex;
        return w;
    }
    function placeAvailableWidth() {
        if (!placeChip || !chromeBar) return 0;
        var brand = root.querySelector('.agri1-chrome-brand');
        var icons = chromeTools ? chromeTools.querySelector('.agri1-chrome-icons') : null;
        var account = chromeTools ? chromeTools.querySelector('.agri1-chrome-account') : null;
        var toolsGap = chromeTools
            ? (parseFloat(window.getComputedStyle(chromeTools).columnGap || window.getComputedStyle(chromeTools).gap) || 8)
            : 8;
        var siblings = 0;
        var gaps = 0;
        if (icons) {
            siblings += icons.offsetWidth;
            gaps += 1;
        }
        if (account) {
            siblings += account.offsetWidth;
            gaps += 1;
        }
        var wrapped = brand && chromeTools && brand.offsetTop !== chromeTools.offsetTop;
        if (wrapped && chromeTools) {
            return chromeTools.clientWidth - siblings - (toolsGap * gaps);
        }
        var barStyle = window.getComputedStyle(chromeBar);
        var pad = (parseFloat(barStyle.paddingLeft) || 0) + (parseFloat(barStyle.paddingRight) || 0);
        var barGap = parseFloat(barStyle.columnGap || barStyle.gap) || 8;
        var brandW = brand ? brand.offsetWidth : 0;
        return chromeBar.clientWidth - pad - barGap - brandW - siblings - (toolsGap * gaps);
    }
    function fitPlaceChip() {
        if (!placeChip) return;
        placeChip.classList.remove('is-hide-ostan', 'is-hide-city');
        if (placeNaturalWidth() > placeAvailableWidth() + 1) {
            placeChip.classList.add('is-hide-ostan');
        }
        if (placeNaturalWidth() > placeAvailableWidth() + 1) {
            placeChip.classList.add('is-hide-city');
        }
    }
    function scheduleFitPlace() {
        if (placeFitRaf) return;
        placeFitRaf = window.requestAnimationFrame(function () {
            placeFitRaf = 0;
            fitPlaceChip();
        });
    }
    ...
    if (document.fonts && document.fonts.ready) {
        document.fonts.ready.then(scheduleFitPlace);
    }
    if (window.ResizeObserver && chromeBar) {
        new ResizeObserver(scheduleFitPlace).observe(chromeBar);
        if (chromeTools) new ResizeObserver(scheduleFitPlace).observe(chromeTools);
    } else {
        window.addEventListener('resize', scheduleFitPlace);
    }
})();
</script>
