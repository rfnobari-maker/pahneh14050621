...
$query = "SELECT count(id) as count  FROM  users  WHERE  id_ostan = '$id_ostan' and id_city = '$id_city'  and S_access = '1' and con_city='1' " ;
$stmt = $dbh->prepare($query);
$stmt->execute();
$row = $stmt->fetch(PDO::FETCH_ASSOC);
$count_finish_mor = $row['count'];
...
{
include('../login/config.php');
$query = "SELECT count(id) as count FROM  users WHERE  id_ostan = '$id_ostan'  and id_city = '$id_city'  and s_access = '1'" ;
$stmt = $dbh->prepare($query);
$stmt->execute();
$row = $stmt->fetch(PDO::FETCH_ASSOC);
