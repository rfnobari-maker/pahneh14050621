...
{
include('../login/config.php');
 $query = "SELECT count(*) FROM Agri1395_1396 WHERE  id_ostan = '$id_ostan' and $v_date_s1 and $v_date_s2 " ;
$query = "SELECT COUNT(DISTINCT Agri_id) FROM  Agri_prod1395_1396 WHERE  id_ostan = '$id_ostan' and $v_date_s1 and $v_date_s2 " ;
$stmt = $dbh->prepare($query);
$stmt->execute();
$count_Agri95 = $stmt->fetchColumn();
$query = "SELECT COUNT(DISTINCT Agri_id) FROM  Agri_prod1396_1397 WHERE   id_ostan = '$id_ostan' and $v_date_s1 and $v_date_s2 " ;
$stmt = $dbh->prepare($query);
$stmt->execute();
