...
include('../login/config.php');
$query = "SELECT * FROM markers WHERE id_city='$id_city'";
$stmt = $dbh->prepare($query);
$stmt->execute();
// $row حلقه ای
...
// Iterates through the MySQL results, creating one Placemark for each row.
 foreach($stmt as $row){
  ...
  $pointNode->appendChild($coorNode);
    ...
    $dom->save('test_city.kml'); // save as file
 	?>
	 <form name="myform1" class="myform" method="post" action="../Cities/map_city.php">
     </form>
