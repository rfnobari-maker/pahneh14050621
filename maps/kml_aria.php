...
//$query = "SELECT * FROM markers WHERE id_city='07'";
$query = "SELECT markers.*
FROM markers
INNER JOIN aria ON markers.id_city = aria.id_city
WHERE aria.id_aria =  '$id_aria' and aria.id_ostan=$id_ostan "  ;
...
// Iterates through the MySQL results, creating one Placemark for each row.
 foreach($stmt as $row){
  ...
  $nameNode = $dom->createElement('name',(city_mar($row['id_mar']).' - مرکز جهاد کشاورزی '.$row['mar']));
  ...
  $pointNode->appendChild($coorNode);
    ...
    $dom->save('test1.kml'); // save as file
 	?>
	 <form name="myform1" class="myform" method="post" action="../Expert_aria/map.php">
     </form>
