...
function processRecords($limit, $max_records) {
    ...
    try {
        $stmt = $dbh->prepare("SELECT id, bah_cod_m AS national_code, tel_m AS mobile_number 
                              FROM bah 
                              WHERE valid NOT IN ('200','600') AND ok = '1' 
                              LIMIT :limit");
        $stmt->bindValue(':limit', $batch_size, PDO::PARAM_INT);
        $stmt->execute();
        $records = $stmt->fetchAll(PDO::FETCH_ASSOC);

        $updateStmt1 = $dbh->prepare("UPDATE bah20 SET valid = :shahkarStatus WHERE bah_cod_m = :bah_cod_m");
        $updateStmt2 = $dbh->prepare("UPDATE bah SET valid = :shahkarStatus WHERE id = :id");
...
$batchSize = 100; // تعداد رکوردها در هر بسته
