<?php
/**
 * داشبورد باغبانی — لایهٔ دوم خانه
 */
...
dash_filter_prg(array(
    ...
    'id_city' => 'id_city',
    'id_mar' => 'id_mar',
    'group_cod' => 'garden_group_cod',
    'product_cod' => 'garden_product_cod',
    'water' => 'garden_water',
    'fruit' => 'garden_fruit',
));
...
$boot_group = isset($dash_boot['garden_group_cod']) ? trim((string) $dash_boot['garden_group_cod']) : '';
$boot_prod = isset($dash_boot['garden_product_cod']) ? trim((string) $dash_boot['garden_product_cod']) : '';
$boot_water = isset($dash_boot['garden_water']) ? trim((string) $dash_boot['garden_water']) : 'all';
$boot_fruit = isset($dash_boot['garden_fruit']) ? trim((string) $dash_boot['garden_fruit']) : 'all';
if ($boot_fruit !== 'baror' && $boot_fruit !== 'nonbaror') {
    $boot_fruit = 'all';
}
...
function dash_garden_home_href($year, $id_ostan, $id_city, $id_mar)
{
    return 'index.php';
}

$homeHref = dash_garden_home_href($boot_year, $boot_ostan, $boot_city, $boot_mar);
?>
<!DOCTYPE html>
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .chart-box.is-share { height: 228px; }
        .dash-chart-legend {
            display: grid; grid-template-columns: minmax(0, 1fr) minmax(0, 1fr);
            gap: 6px 8px; margin: 8px 0 0; padding: 0; list-style: none;
        ...
        .dash-chart-legend li {
            ...
            font-size: 11px; color: #14221A; line-height: 1.3;
        }
        .dash-chart-legend i {
        ...
        .dash-chart-legend b {
            font-size: 11px; font-weight: 700; color: #1F6B45; white-space: nowrap;
...
<?php endif; ?>
        ...
        <div class="dash-filters">
            ...
            <div class="dash-tool">
                <span class="dash-pills-label" id="fruit-label">بارور / غیربارور</span>
                <div class="dash-pills" role="group" aria-labelledby="fruit-label">
                    <button type="button" class="dash-pill is-on" data-fruit="all">هر دو</button>
                    <button type="button" class="dash-pill" data-fruit="baror">بارور</button>
                    <button type="button" class="dash-pill" data-fruit="nonbaror">غیربارور</button>
                </div>
            </div>
        </div>
        ...
        <p class="dash-status" id="dash-status" role="status">در حال بارگذاری آمار باغبانی...</p>
        ...
        <div class="dash-grid">
            <aside class="dash-col">
                ...
                <section class="dash-card">
                    ...
                    <div class="kpi is-wide"><b id="k-plant">۰</b><span>کل سطح</span></div>
                    <div class="kpi-row">
                ...
                <section class="dash-card">
                    ...
                    <div class="kpi is-wide">
                        <b id="k-baror">۰</b>
                        <span>بارور</span>
                        <em id="k-baror-pct"></em>
                    </div>
                    <div class="kpi-row">
                        <div class="kpi"><b id="k-baror-abi">۰</b><span>بارور آبی</span></div>
                        <div class="kpi"><b id="k-baror-dim">۰</b><span>بارور دیم</span></div>
                    </div>
                    <div class="kpi-row">
                        <div class="kpi"><b id="k-nonbaror">۰</b><span>غیربارور</span></div>
                        <div class="kpi"><b id="k-trees">۰</b><span>درخت بارور (هزار اصله)</span></div>
                    </div>
                </section>
                <section class="dash-card">
                    <div class="dash-card-head">
                    ...
                    <div class="kpi is-wide"><b id="k-prod">۰</b><span>تولید قطعی</span></div>
                    <div class="kpi-row">
                        <div class="kpi"><b id="k-pred">۰</b><span>پیش‌بینی</span></div>
                        <div class="kpi"><b id="k-yield">—</b><span>عملکرد (کیلوگرم در هکتار بارور)</span></div>
                    </div>
                </section>
                <section class="dash-card">
                    <div class="dash-card-head">
                        <h2 id="water-title">بارور و غیربارور</h2>
                        <span class="dash-card-tag">سهم</span>
                    </div>
                    <div class="chart-box"><canvas id="chart-water" aria-label="نمودار بارور و غیربارور"></canvas></div>
                </section>
            </aside>
            ...
            <section class="dash-card dash-map-wrap">
                ...
                <div class="dash-rank-metrics" role="tablist" aria-label="شاخص نقشه">
                    <button type="button" class="dash-rank-metric is-on" data-metric="baror">بارور</button>
                    <button type="button" class="dash-rank-metric" data-metric="nonbaror">غیربارور</button>
                    <button type="button" class="dash-rank-metric" data-metric="cut">برش</button>
                </div>
                <div id="dash-map" role="application" aria-label="نقشه باغبانی"></div>
                ...
                <section class="dash-rank" id="dash-rank">
                    ...
                    <p class="dash-rank-hint" id="dash-rank-hint">سطح به هکتار است. زیر کشت = بارور + غیربارور.</p>
            ...
            <aside class="dash-col">
                ...
                <section class="dash-card">
                    ...
                    <div class="kpi-row">
                        <div class="kpi"><b id="k-plan-baror">۰</b><span>بارور ابلاغی</span></div>
                        <div class="kpi"><b id="k-plan-nonbaror">۰</b><span>غیربارور ابلاغی</span></div>
                    </div>
                </section>
                <section class="dash-card">
                    <div class="dash-card-head">
                        <h2>وضعیت برش الگو</h2>
                        <span class="dash-card-tag is-plan">شهرستانی</span>
                    </div>
                    <div class="kpi is-wide"><b id="k-cut">۰</b><span>سطح برش شهرستانی (هکتار)</span></div>
                    <div class="kpi-row">
                        <div class="kpi"><b id="k-cut-abi">۰</b><span>آبی</span></div>
                        <div class="kpi"><b id="k-cut-dim">۰</b><span>دیم</span></div>
                    </div>
                    <div class="kpi-row">
                        <div class="kpi"><b id="k-cut-baror">۰</b><span>بارور برش</span></div>
                        <div class="kpi"><b id="k-cut-nonbaror">۰</b><span>غیربارور برش</span></div>
                    </div>
                    <div class="kpi is-wide">
                        <b id="k-realize">—</b>
                        <span id="k-realize-label">برش شهرستانی نسبت به ابلاغی</span>
                    </div>
                ...
                <section class="dash-card">
                    ...
                    <div class="chart-box is-share"><canvas id="chart-share" aria-label="نمودار سهم"></canvas></div>
                    <ul class="dash-chart-legend" id="share-legend" hidden></ul>
                </section>
                <section class="dash-card">
                    <div class="dash-card-head">
                        <h2>کشت، ابلاغی، برش</h2>
                        <span class="dash-card-tag">هکتار</span>
                    </div>
...
(function () {
    ...
    var state = {
        ...
        fruit: <?php echo json_encode(($boot_fruit === 'baror' || $boot_fruit === 'nonbaror') ? $boot_fruit : 'all'); ?>,
        metric: 'baror',
        wantLive: false,
    ...
    function renderSourceBar(d) {
        ...
        var src = (d && d.data_source) ? d.data_source : 'live';
        if (!d || (d.level !== 'country' && d.level !== 'ostan')) {
            bar.hidden = true;
            btn.hidden = true;
            return;
        }
        ...
        if (src === 'snap') {
            ...
            meta.textContent = d.built_at
                ...
                : (d.level === 'ostan' ? 'آمار این استان از جدول اسنپ' : 'آمار باغبانی کشور از جدول اسنپ');
        } else if (src === 'live') {
            ...
            tag.className = 'dash-source-tag is-live';
            meta.textContent = 'اسنپ‌شات باغبانی برای این سال یافت نشد';
        }
        btn.hidden = !(d.level === 'country' && d.can_live && src !== 'live');
        btn.disabled = false;
    }
    ...
    function placeValue(p, metric) {
        if (metric === 'baror' || metric === 'harvest') return Number(p.baror) || 0;
        if (metric === 'nonbaror') return Number(p.nonbaror) || 0;
        if (metric === 'cut') return Number(p.cut) || 0;
    ...
    function drawDoughnut(id, labels, data, colors, showLegend) {
        killChart(id);
    ...
    function packSlices(labels, data, maxN) {
        var items = [];
        (labels || []).forEach(function (label, i) {
            var value = Number(data[i]) || 0;
            if (value > 0) items.push({ label: label, value: value });
        });
        if (items.length <= maxN) return items;
        var head = items.slice(0, maxN - 1);
        var rest = 0;
        items.slice(maxN - 1).forEach(function (x) { rest += x.value; });
        head.push({ label: 'سایر', value: rest });
        return head;
    }
    function renderShareLegend(items, pal) {
        var ul = document.getElementById('share-legend');
        if (!ul) return;
        ul.innerHTML = '';
        var tot = 0;
        items.forEach(function (x) { tot += x.value; });
        items.forEach(function (x, i) {
            var li = document.createElement('li');
            var pct = tot > 0 ? Math.round((x.value * 1000) / tot) / 10 : 0;
            var name = toFaDigits(x.label || '');
            li.innerHTML =
    ...
    function setKpi() {
        ...
        document.getElementById('k-baror').textContent = faHa(baror.total);
        document.getElementById('k-baror-abi').textContent = faHa(baror.abi);
        document.getElementById('k-baror-dim').textContent = faHa(baror.dim);
        document.getElementById('k-nonbaror').textContent = faHa(nonbaror.total);
        document.getElementById('k-trees').textContent = faHa(trees.baror);
        var hp = document.getElementById('k-baror-pct');
        hp.textContent = (k.baror_pct != null) ? ('سهم از سطح: ' + faNum(k.baror_pct) + '٪') : '';
        ...
        document.getElementById('k-plan-baror').textContent = faHa(plan.baror);
        document.getElementById('k-plan-nonbaror').textContent = faHa(plan.nonbaror);
        var cut = k.cut || {};
        document.getElementById('k-cut').textContent = faHa(cut.total);
        document.getElementById('k-cut-abi').textContent = faHa(cut.abi);
        document.getElementById('k-cut-dim').textContent = faHa(cut.dim);
        document.getElementById('k-cut-baror').textContent = faHa(cut.baror);
        document.getElementById('k-cut-nonbaror').textContent = faHa(cut.nonbaror);
        ...
        var lab = {
            none: 'ابلاغی یا برش شهرستانی برای این فیلتر نیست',
            under: 'برش کمتر از ابلاغی',
            near: 'برش نزدیک به ابلاغی',
            over: 'برش بیش از ابلاغی'
        };
        document.getElementById('k-realize-label').textContent = lab[st] || lab.none;
        if (state.fruit === 'all') {
            drawDoughnut('chart-water', ['بارور', 'غیربارور'], [baror.total || 0, nonbaror.total || 0], ['#1F6B45', '#A16207']);
            var wt = document.getElementById('water-title');
            if (wt) wt.textContent = 'بارور و غیربارور';
        } else {
            drawDoughnut('chart-water', ['آبی', 'دیم'], [plant.abi || 0, plant.dim || 0], ['#1F6B45', '#A16207']);
            var wt2 = document.getElementById('water-title');
            if (wt2) wt2.textContent = 'آبی و دیم';
        }
        var share = last.share || [];
        var sl = share.map(function (s) { return s.label; });
        var sd = share.map(function (s) { return Number(s.plant) || 0; });
        var titles = { group: 'سهم گروه‌ها', product: 'سهم محصولات گروه', water: 'آبی و دیم', fruit: 'بارور و غیربارور' };
        ...
        var packed = packSlices(sl, sd, 6);
        drawDoughnut('chart-share', packed.map(function (x) { return x.label; }), packed.map(function (x) { return x.value; }), pal, false);
        renderShareLegend(packed, pal);
        drawBar('chart-compare', ['کشت', 'ابلاغی', 'برش'], [{
            data: [plant.total || 0, plan.total || 0, (cut.total || 0)],
            backgroundColor: ['#1F6B45', '#C9A227', '#A16207']
        }]);
        scheduleFit();
    }
    ...
    function apply(d) {
        ...
        document.querySelectorAll('[data-fruit]').forEach(function (b) {
            b.classList.toggle('is-on', b.getAttribute('data-fruit') === state.fruit);
        });
        ...
        if (d.missing_plan) bits.push('جدول ابلاغی استان (Garden_ab_ostan) موجود نیست.');
        if (d.missing_cut) bits.push('جدول برش شهرستانی (Garden_ab_city) موجود نیست.');
        var hints = {
    ...
    document.querySelectorAll('[data-fruit]').forEach(function (b) {
        b.addEventListener('click', function () {
            state.fruit = b.getAttribute('data-fruit') || 'all';
            load({});
        });
    });
    document.querySelectorAll('[data-metric]').forEach(function (b) {
        b.addEventListener('click', function () {
            state.metric = b.getAttribute('data-metric') || 'baror';
