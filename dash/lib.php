...
if (!function_exists('dash_fa_digits')) {
    function dash_fa_digits($s)
    {
...
if (!function_exists('dash_norm_fa')) {
...
if (!function_exists('dash_req')) {
    function dash_req($key, $default = '')
    {
        if (isset($_POST[$key])) {
            return $_POST[$key];
        }
...
if (!function_exists('dash_req_trim')) {
    function dash_req_trim($key, $default = '')
    {
        $v = dash_req($key, $default);
        if (is_array($v)) {
            return $default;
        }
...
if (!function_exists('dash_require_post')) {
    function dash_require_post()
    {
        $m = isset($_SERVER['REQUEST_METHOD']) ? strtoupper((string) $_SERVER['REQUEST_METHOD']) : '';
        if ($m === 'POST') {
            return;
        }
        dash_json_out(array('ok' => false, 'error' => 'درخواست نامعتبر است.'));
        exit;
    }
}

if (!function_exists('dash_filter_defaults')) {
    function dash_filter_defaults()
    {
        return array(
            'year' => '',
            'id_ostan' => '',
...
if (!function_exists('dash_filter_session_get')) {
    function dash_filter_session_get()
    {
        $out = dash_filter_defaults();
        if (empty($_SESSION['dash_filter']) || !is_array($_SESSION['dash_filter'])) {
            return $out;
        }
        foreach ($_SESSION['dash_filter'] as $k => $v) {
            $out[$k] = $v;
        }
        return $out;
    }
...
if (!function_exists('dash_filter_session_set')) {
    function dash_filter_session_set($parts)
    {
        if (!is_array($parts) || !$parts) {
            return;
        }
        $cur = dash_filter_session_get();
        foreach ($parts as $k => $v) {
            $cur[$k] = $v;
        }
        $_SESSION['dash_filter'] = $cur;
    }
}

if (!function_exists('dash_filter_prg')) {
    function dash_filter_prg($map)
    {
        $m = isset($_SERVER['REQUEST_METHOD']) ? strtoupper((string) $_SERVER['REQUEST_METHOD']) : '';
        if ($m !== 'POST') {
            return;
        ...
        $parts = array();
        $hit = false;
        foreach ($map as $postKey => $sessKey) {
            if (is_int($postKey)) {
                $postKey = $sessKey;
            }
        ...
        $self = basename(isset($_SERVER['SCRIPT_NAME']) ? (string) $_SERVER['SCRIPT_NAME'] : 'index.php');
        if ($self === '') {
            $self = 'index.php';
        }
...
function dash_force_ostan()
{
    global $dash_force_ostan;
    return (isset($dash_force_ostan) && $dash_force_ostan !== '') ? trim($dash_force_ostan . '') : '';
}

function dash_clamp_geo(&$id_ostan, &$id_city, &$id_mar)
{
    $force = dash_force_ostan();
    if ($force === '') {
        return '';
    }
    $id_ostan = trim($id_ostan . '');
    $id_city = trim($id_city . '');
    $id_mar = trim($id_mar . '');
    if (!dash_ostan_same($id_ostan, $force)) {
        $id_ostan = $force;
        $id_city = '';
        $id_mar = '';
        return $force;
...
function dash_geo_level($id_ostan, $id_city, $id_mar)
{
    if ($id_mar !== '') {
        return 'mar';
    }
...
function dash_geo_crumb($dbh, $id_ostan, $id_city, $id_mar, $force_ostan)
{
    $crumb = array();
    if ($force_ostan === '') {
        $crumb[] = array('level' => 'country', 'label' => 'کشور', 'id_ostan' => '', 'id_city' => '', 'id_mar' => '');
    }
    if ($id_ostan !== '') {
...
function dash_bah_snap_ensure($dbh)
{
    if (!$dbh) {
        return false;
    }
    if (dash_table_exists($dbh, 'dash_snap_bah')) {
        return true;
    }
    $sql = "CREATE TABLE IF NOT EXISTS `dash_snap_bah` (
    ...
    return dash_table_exists($dbh, 'dash_snap_bah');
}

function dash_bah_snap_pack($row)
{
    return array(
        'total' => (int) $row['bah_total'],
        'natural' => (int) $row['bah_natural'],
        'legal' => (int) $row['bah_legal'],
        'male' => (int) $row['bah_male'],
        'female' => (int) $row['bah_female']
    );
}

function dash_bah_empty()
{
    return array(
        'total' => 0,
        'natural' => 0,
...
function dash_bah_snap_get($dbh, $level, $id_ostan)
{
    if ($level !== 'country' && $level !== 'ostan') {
        return null;
    }
    $id = ($level === 'country') ? '' : trim($id_ostan . '');
    $rows = dash_rows(
        $dbh,
        "SELECT bah_total, bah_natural, bah_legal, bah_male, bah_female, built_at
         FROM dash_snap_bah
         WHERE level_code = ? AND id_ostan = ?
         LIMIT 1",
        array($level, $id)
    );
    if (!$rows) {
    ...
    return dash_bah_snap_pack($rows[0]);
}

function dash_bah_view($dbh, $level, $id_ostan, $id_city, $id_mar, $snapBah = null)
{
    if ($level === 'country' || $level === 'ostan') {
        $id = ($level === 'country') ? '' : trim($id_ostan . '');
        $rows = dash_rows(
        ...
        if ($rows) {
            return array(
                'bah' => dash_bah_snap_pack($rows[0]),
                'bah_source' => 'snap',
                'bah_built_at' => isset($rows[0]['built_at']) ? $rows[0]['built_at'] : null
            );
        }
        if (!empty($snapBah) && is_array($snapBah) && isset($snapBah['total'])) {
            return array(
                'bah' => array(
                    'total' => (int) $snapBah['total'],
                    'natural' => isset($snapBah['natural']) ? (int) $snapBah['natural'] : 0,
                    'legal' => isset($snapBah['legal']) ? (int) $snapBah['legal'] : 0,
                    'male' => isset($snapBah['male']) ? (int) $snapBah['male'] : 0,
                    'female' => isset($snapBah['female']) ? (int) $snapBah['female'] : 0
                ),
                'bah_source' => 'snap',
                'bah_built_at' => null
            );
...
function dash_bah_for_view($dbh, $level, $id_ostan, $id_city, $id_mar, $snapBah = null)
{
    $view = dash_bah_view($dbh, $level, $id_ostan, $id_city, $id_mar, $snapBah);
    return $view['bah'];
}

function dash_bah_snap_upsert($dbh, $level, $id_ostan, $name, $stats, $builtAt, $buildMs)
{
    $sql = "INSERT INTO dash_snap_bah
            (level_code, id_ostan, name_label, bah_total, bah_natural, bah_legal, bah_male, bah_female, built_at, build_ms)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ON DUPLICATE KEY UPDATE
              name_label=VALUES(name_label),
              bah_total=VALUES(bah_total),
              bah_natural=VALUES(bah_natural),
              bah_legal=VALUES(bah_legal),
              bah_male=VALUES(bah_male),
              bah_female=VALUES(bah_female),
...
function dash_bah_snap_build($dbh)
{
    $out = array('ok' => false, 'rows' => 0, 'ms' => 0, 'errors' => array(), 'total' => 0);
    if (!dash_bah_snap_ensure($dbh)) {
        $out['errors'][] = 'cannot create dash_snap_bah';
        return $out;
    }
    $t0 = microtime(true);
    ...
    $names = array();
    $nRows = dash_rows($dbh, 'SELECT id_ostan, ostan FROM ostanname', array());
    foreach ($nRows as $n) {
        $names[trim($n['id_ostan'] . '')] = $n['ostan'];
    }
    $country = array('total' => 0, 'natural' => 0, 'legal' => 0, 'male' => 0, 'female' => 0);
    foreach ($groups as $g) {
        $idO = trim($g['id_ostan'] . '');
        $stats = array(
            'total' => (int) $g['total'],
            'natural' => (int) $g['natural_c'],
            'legal' => (int) $g['legal_c'],
            'male' => (int) $g['male_c'],
            'female' => (int) $g['female_c']
        );
        $country['total'] += $stats['total'];
        $country['natural'] += $stats['natural'];
        $country['legal'] += $stats['legal'];
        $country['male'] += $stats['male'];
        $country['female'] += $stats['female'];
        $label = isset($names[$idO]) ? $names[$idO] : $idO;
        if (dash_bah_snap_upsert($dbh, 'ostan', $idO, $label, $stats, $builtAt, 0)) {
            $out['rows']++;
        } else {
            $out['errors'][] = 'upsert ostan ' . $idO . ' failed';
        }
    }
    $ms = (int) round((microtime(true) - $t0) * 1000);
    if (dash_bah_snap_upsert($dbh, 'country', '', 'Iran', $country, $builtAt, $ms)) {
        $out['rows']++;
    } else {
        $out['errors'][] = 'upsert country failed';
    }
    $out['ms'] = $ms;
    $out['total'] = $country['total'];
...
function dash_cities($dbh, $id_ostan, $year = 0, $withCoords = true)
{
    if (!$year) {
        ...
        if (!$cols) {
            $dbh->exec(
                "ALTER TABLE `$table`
                 ...
                 COMMENT 'JSON: garden_area detail, rank'
                 AFTER `visits_text`"
            );
        }
        $done[$table] = true;
...
/** Extended yearly snap fields (garden/rank). Bah lives in dash_snap_bah, not here. */
...
/**
 * Country/ostan from year-independent dash_snap_bah; city/mar live.
 */
function dash_bah_from_open_or_live($dbh, $level, $id_ostan, $id_city, $id_mar, $preferYear = 0)
{
    return dash_bah_for_view($dbh, $level, $id_ostan, $id_city, $id_mar, null);
}
