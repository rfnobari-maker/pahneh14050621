 ...
 * Open while logged in to dash:
 *   /dash/dash_year_status_seed_run.php?confirm=1
 */
...
";
    ...
    // Always include open years even if missing from b_sal
    foreach (array('1404', '1405') as $must) {
        if (!in_array($must, $salList, true)) {
            $salList[] = $must;
        }
    }
    sort($salList);
