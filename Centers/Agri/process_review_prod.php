...
try {
    $stmt = $dbh->prepare("SELECT * FROM Agri_prod_req WHERE id = :id AND status = 0");
...
// بررسی اینکه آیا کد محصول تغییر کرده و سطح زیر کشت افزایش یافته    
   ...
   if ( $cod_mah != $new_cod_mah or $zer_kesht < $new_zer_kesht)
   {
    ...
    $req_data['Agri_id'],           // Agri_id
    $req_data['new_cod_mah'],       // محصول جدید
    $new_zer_kesht,                 // سطح جدید
    $zer_kesht,                     // سطح قبلی ← این را اضافه کن
    $req_data['z_sal'], 
    $geo_data['id_ostan'], 
    $geo_data['id_city'], 
    $geo_data['id_mar'], 
    ...
    $sql_update = "UPDATE Agri_prod_req SET status = :status, center_comment = :comment, center_date = :mdate WHERE id = :id";
    ...
    if ($result) {
        $msg = ($final_status == 1) ? 'درخواست تایید و به مدیر شهرستان ارسال شد.' : 'درخواست رد شد.';
