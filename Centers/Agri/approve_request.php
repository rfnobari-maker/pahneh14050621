...
include('../../lock_p2.php'); // اینجا متغیرهای id_ostan, id_city, id_mar را دریافت می‌کنیم
...
// بررسی فیلدهای ضروری
if (empty($data['username']) || empty($data['mor_cod_m']) || empty($data['z_sal'])) {
    $response = array(
        'status' => 'error',
        'message' => 'نام کاربری، کد ملی کارشناس یا سال زراعی وجود ندارد'
    );
    echo json_encode($response);
    exit;
}
...
try {
    ...
    // سپس جدول Agri_prod1403_1404 را آپدیت می‌کنیم
    $updateStmt = $dbh->prepare("UPDATE $Agri_prod_table SET app_level = :app_level, app_date = :app_date WHERE mor_cod_m = :mor_cod_m and app_level in ('','10')");
    $updateStmt->bindParam(':app_level', $app_level_v);
    $updateStmt->bindParam(':app_date', $app_date);
    $updateStmt->bindParam(':mor_cod_m', $mor_cod_m);
    $updateStmt->execute();
