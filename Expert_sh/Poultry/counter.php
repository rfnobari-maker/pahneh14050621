...
$query = "SELECT id FROM  bee WHERE id_mar = '$id_mar'  and sal = '1404' " ;
...
{
include('../../login/config.php');
$query = "SELECT SUM(tk_bo) AS kol_k_bo from bee WHERE id_mar = '$id_mar'  and sal = '1404' ";
$stmt = $dbh->prepare($query);
$stmt->execute();
...
{
include('../../login/config.php');
$query = "SELECT SUM(tk_mo) AS kol_k_mo from bee WHERE id_mar = '$id_mar'  and sal = '1404' ";
$stmt = $dbh->prepare($query);
$stmt->execute();
$row = $stmt->fetch(PDO::FETCH_ASSOC);
