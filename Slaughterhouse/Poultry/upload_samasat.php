...
<table width="949" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
  <tr>
    <td><img src="../../files/images/head009_241.png" width="964" height="140" /></td>
  </tr>
  <tr>
    <td></td>
  </tr>
...
if (isset($_GET['function'])){
  ...
  <tr>
    <td  height="100px"colspan="2" valign="middle" background="../../files/bottom.gif"><p class="MenuItemRight">Copyright © 2021, سازمان جهاد کشاورزی آذربایجان شرقی All rights   reserved</p>
...
include('../../login/config.php');
 $query = "DELETE FROM `samasat` WHERE `z_kol` = 0 or `age_day` > 60 or `age_day` = 0"; 
 $stmt = $dbh->prepare($query);
 $stmt->execute();

 $query = "DELETE FROM `samasat2` WHERE 1 "; 
 $stmt = $dbh->prepare($query);
 $stmt->execute();

 $query = "INSERT INTO `samasat2`(`rad`, `ostan`, `city`, `cod_sys`, `cod_p`, `cod_ep`, `sh_yek`, `name_unit`, `name_m`, `z_kol`, `sh_moj`, `sh_gova`, `no_dar`, `date_joj`, `date_part`, `no_joj`, `age_day`, `age_w`, `no_part`, `no_ert`, `tavoni`) 
SELECT *   FROM `samasat`
"; 
 $stmt = $dbh->prepare($query);
 $stmt->execute();

$query = "UPDATE samasat2
INNER JOIN samasat_cityname ON samasat2.city = samasat_cityname.city
SET samasat2.id_ostan = samasat_cityname.id_ostan, 
    samasat2.id_city = samasat_cityname.id_city , samasat2.m_joj = samasat2.no_joj
WHERE 1 "; 
 $stmt = $dbh->prepare($query);
 $stmt->execute();
//

$query = "SELECT SUM( t_kasr ) as kasr , `sh_yek` , `date_joj` 
FROM `samasat_event`
WHERE 1
GROUP BY `sh_yek` , `date_joj` " ; 
$stmt = $dbh->prepare($query);
$stmt->execute();
...
$query = "UPDATE `samasat2` SET `m_joj` = m_joj - $kasr  where `sh_yek` = $sh_yek and `date_joj` = '$date_joj' "; 
//alert($query) ; 
$stmt = $dbh->prepare($query);
$stmt->execute();
}
