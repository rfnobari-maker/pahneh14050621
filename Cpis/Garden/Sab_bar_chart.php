...
<div class="container">
    ...
    if (!empty($z_sal)) {
        $dbh = get_db_connection();
        if ($dbh) {
            // Query to get the total official cultivation area (s_bar_dem and s_bar_abi) and production (t_dem and t_abi)
            $query_sum_ostan = "
                SELECT 
                    SUM(s_bar_dem) AS total_s_bar_dem, 
                    SUM(s_bar_abi) AS total_s_bar_abi,
            ...
            $total_s_bar_dem = $sum_ostan_row ? (float)$sum_ostan_row['total_s_bar_dem'] : 0;
            $total_s_bar_abi = $sum_ostan_row ? (float)$sum_ostan_row['total_s_bar_abi'] : 0;
            ...
            if ($stmt && $stmt->rowCount() > 0) {
                ...
                $s_bar_dem_data = array();
                $s_bar_abi_data = array();
                $t_dem_data = array();
                $t_abi_data = array();

                foreach($stmt as $row) {
                    $province_names[] = $row['ostan'];
                    $s_bar_dem_data[] = (float)$row['total_s_bar_dem'];
                    $s_bar_abi_data[] = (float)$row['total_s_bar_abi'];
                ...
                $s_bar_dem_data_json = json_encode($s_bar_dem_data);
                $s_bar_abi_data_json = json_encode($s_bar_abi_data);
                $t_dem_data_json = json_encode($t_dem_data);
                $t_abi_data_json = json_encode($t_abi_data);
                
                // Total values
                $total_s_bar_abi_json = json_encode($total_s_bar_abi);
                $total_s_bar_dem_json = json_encode($total_s_bar_dem);
                $total_t_abi_json = json_encode($total_t_abi);
                $total_t_dem_json = json_encode($total_t_dem);
    ?>
    <div class="card">
        <h2 class="chart-title">نمودار مقایسه‌ای سطح بارور (آبی و دیم)</h2>
...
const s_bar_dem_data = <?php echo $s_bar_dem_data_json; ?>;
const s_bar_abi_data = <?php echo $s_bar_abi_data_json; ?>;
const t_dem_data = <?php echo $t_dem_data_json; ?>;
const t_abi_data = <?php echo $t_abi_data_json; ?>;

const total_s_bar_abi = <?php echo $total_s_bar_abi_json; ?>;
const total_s_bar_dem = <?php echo $total_s_bar_dem_json; ?>;
...
// Create the charts
createChart('sChart', 'نمودار سطح', s_bar_dem_data, s_bar_abi_data, total_s_bar_dem, total_s_bar_abi, 'هکتار');
