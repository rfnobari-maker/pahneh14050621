...
function execute_prepared_statement($query, $params) {
    ...
    // Use PDO::prepare for safer handling of SQL
    $stmt = $dbh->prepare($query);
    
    // Bind parameters safely, ensuring correct handling for PHP 5.2 compatibility attempt 
    // (though real PDO support in 5.2.3 is rare, assuming a later version or polyfill is used)
    // The foreach loop from the original code for bindParam is omitted here for simplicity 
    // and modern PDO usage, but if strictly adhering to 5.2.3, the original loop might be needed.
    // For standard PHP:
    $stmt->execute($params);
...
<div class="container">
    ...
    if (!empty($z_sal)) {
        $dbh = get_db_connection();
        if ($dbh) {
            // Query to get the total official cultivation area (s_nobar_dem and s_nobar_abi)
            $query_sum_ostan = "
                SELECT 
                    SUM(s_nobar_dem) AS total_s_nobar_dem, 
                    SUM(s_nobar_abi) AS total_s_nobar_abi
            ...
            $total_s_nobar_dem = $sum_ostan_row ? (float)$sum_ostan_row['total_s_nobar_dem'] : 0;
            $total_s_nobar_abi = $sum_ostan_row ? (float)$sum_ostan_row['total_s_nobar_abi'] : 0;
            // Production variables removed: $total_t_dem and $total_t_abi

            // Query to get province-specific data for Area (Surface)
            $query = "
                SELECT 
                    osn.ostan,
            ...
            if ($stmt && $stmt->rowCount() > 0) {
                ...
                $s_nobar_dem_data = array();
                $s_nobar_abi_data = array();
                // Production data variables removed

                foreach($stmt as $row) {
                    $province_names[] = $row['ostan'];
                    $s_nobar_dem_data[] = (float)$row['total_s_nobar_dem'];
                    $s_nobar_abi_data[] = (float)$row['total_s_nobar_abi'];
                    // Production data assignments removed
                }
                
                // JSON encoding for JavaScript
                $province_names_json = json_encode($province_names);
                $s_nobar_dem_data_json = json_encode($s_nobar_dem_data);
                $s_nobar_abi_data_json = json_encode($s_nobar_abi_data);
                // Production JSON variables removed
                
                // Total values
                $total_s_nobar_abi_json = json_encode($total_s_nobar_abi);
                $total_s_nobar_dem_json = json_encode($total_s_nobar_dem);
                // Total Production JSON variables removed
    ?>
    <div class="card">
        <h2 class="chart-title">نمودار مقایسه‌ای سطح غیر بارور (آبی و دیم)</h2>
...
const s_nobar_dem_data = <?php echo $s_nobar_dem_data_json; ?>;
const s_nobar_abi_data = <?php echo $s_nobar_abi_data_json; ?>;
// Production data variables removed

const total_s_nobar_abi = <?php echo $total_s_nobar_abi_json; ?>;
const total_s_nobar_dem = <?php echo $total_s_nobar_dem_json; ?>;
// Total Production variables removed
...
// Create the charts
createChart('sChart', 'نمودار سطح', s_nobar_dem_data, s_nobar_abi_data, total_s_nobar_dem, total_s_nobar_abi, 'هکتار');
// Production chart creation removed: createChart('tChart', 'نمودار پیش بینی تولید', t_dem_data, t_abi_data, total_t_dem, total_t_abi, 'تن');
