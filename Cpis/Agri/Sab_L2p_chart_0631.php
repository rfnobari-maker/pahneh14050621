...
<html lang="fa" dir="rtl"><head>
    ...
    /* می‌توانید نسبت ابعاد دلخواه را برای فضایی که نمودار اشغال می‌کند، تنظیم کنید */
    /* مثلا 4:3 */
    /* padding-top: 75%; */ 
}
    ...
    if (!empty($mah_qroup) && !empty($mah_name) && !empty($z_sal) && !empty($id_ostan1))
    {
        ...
        if ($dbh) {
            ...
            if ($stmt && $stmt->rowCount() > 0) {
                ...
                $city_ids = array(); // New array to store city IDs
                ...
                foreach($stmt as $row) {
                    ...
                    $city_ids[] = $row['id_city']; // Add city ID to the new array
                }
                ...
                options: {
                    ...
                    onClick: (e) => {
                        ...
                        if (points.length) {
                            ...
                            const id_ostan = '<?php echo $id_ostan1; ?>';							
                            const mah_qroup = '<?php echo $mah_qroup; ?>';
                            const mah_name = '<?php echo $mah_name; ?>';
                            const url = `marakez_chart.php?id_ostan=${id_ostan}&z_sal=${z_sal}&mah_qroup=${mah_qroup}&mah_name=${mah_name}&id_city=${cityId}`;
                            
                            // Modified line to open a popup window with a unique name
                       window.open(url, "marakez_popup", "location=1,status=1,scrollbars=1,width=1000,height=800,top=50,left=50");
                        }
                    },
                    plugins: {
                        legend: {
