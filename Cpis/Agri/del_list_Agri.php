...
foreach($stmt as $row)
{
 ...
 $prod_id = $row['id'] ;
if ($z_sal <> '1403-1404' and check_payesh($prod_id,0,left($zsal,4)) == 2)    ; 
{
?>
...
if ($z_sal <> '1403-1404' and check_payesh($prod_id,0,left($zsal,4)) == 0)    ; 
{
?>
<form name="myform" class="myform" method="post" action="liste_Agri.php?id=<?php echo $id_page .'#1' ?>">
     <input type="hidden" name="com_alert" value="ارتباط با سرور برقرار نشد بعد بررسی کنید ">
     <input type="hidden" name="bah_cod_m" value="<?php echo $bah_cod_m ?>" />
     <input type="hidden" name="action_lise" value="1" />
     <input type="hidden" name="back_p" value="1" />
</form>
