...
// دریافت سال زراعی از POST
$z_sal = isset($_POST['z_sal']) ? $_POST['z_sal'] : null;
// استخراج 4 رقم اول (سال شروع)
$z_sal_year = '';
if ($z_sal) {
    $z_sal_year = substr($z_sal, 0, 4);
}
...
<div class="container">
    ...
    $dbh = get_db_connection();
    if ($dbh) {
        // کوئری عملکرد (تحویلی) با شرط سال
        ...
        // اجرای کوئری عملکرد با پارامتر سال
        $params = array(':year' => $z_sal_year);
        $stmt_perf = execute_prepared_statement($q_perf, $params);
        $stmt_plan = execute_prepared_statement($q_plan, $params);
