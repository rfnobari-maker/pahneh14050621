...
$z_sal_list = array();
 $stmt = execute_prepared_statement("SELECT DISTINCT z_sal FROM sokht_ostan where z_sal = '$z_sal' ORDER BY z_sal DESC", array());
if ($stmt) {
...
if (empty($z_sal_list)) {
    die('<div class="card info-message"><p>هیچ اطلاعاتی برای این سال زراعی در جدول یافت نشد.</p></div>');
}
