...
$Agri_prod_archive = 'Agri_prod1405_1406_uot_ab';
...
foreach ($rows as $row) {
    ...
    $row['is_free'] = $is_free;
    if ($payesh == 2) {
        $row['payesh_label'] = 'نهاده اختصاص داده شده — منتقل نشد';
    } elseif ($payesh == 0 || $payesh == -1) {
        $row['payesh_label'] = 'ارتباط پایش برقرار نشد — منتقل نشد';
    } else {
        $row['payesh_label'] = 'آزاد برای انتقال';
    }
    $checked[] = $row;
    if ($is_free) {
        $free_ids[] = $pid;
...
if ($do_move && !empty($free_ids)) {
    try {
        $dbh->beginTransaction();

        $id_list = implode(',', array_map('intval', $free_ids));
        ...
        ");
        $moved_count = $dbh->exec("
            DELETE FROM `$Agri_prod_table`
            WHERE id IN ($id_list)
        ");
        if ($moved_count === false) {
            $moved_count = 0;
        }
        ...
        sabt_event(
            ...
            $time,
            '',
            'انتقال محصولات خارج از برش به ' . $Agri_prod_archive . ' /' . $z_sal . '/تعداد ' . $moved_count,
            $id_ostan
        );
...
<head>
    ...
    <title>انتقال محصولات خارج از برش ۱۴۰۵–۱۴۰۶</title>
...
<body>
    ...
    <div class="bar">
        ...
        — آزاد برای انتقال: <strong><?php echo (int)$free_count; ?></strong>
        — قفل پایش: <strong><?php echo (int)($total_count - $free_count); ?></strong>
    </div>

    <?php if ($do_move && $move_error === '') { ?>
        <p class="msg msg-ok"><?php echo (int)$moved_count; ?> ردیف به <?php echo doph_h($Agri_prod_archive); ?> منتقل و از جدول اصلی حذف شد. t_mah به‌روز شد.</p>
    <?php } ?>
    <?php if ($move_error !== '') { ?>
        <p class="msg msg-err">خطا در انتقال: <?php echo doph_h($move_error); ?></p>
    <?php } ?>

    <?php if (!$do_move && $free_count > 0) { ?>
        <form method="post" onsubmit="return confirm('ردیف‌های آزاد ابتدا به جدول آرشیو کپی و سپس از جدول اصلی حذف می‌شوند. ادامه می‌دهید؟');">
            <input type="hidden" name="action" value="move"/>
            <p><button type="submit">انتقال و حذف <?php echo (int)$free_count; ?> ردیف آزاد</button></p>
        </form>
    <?php } ?>
