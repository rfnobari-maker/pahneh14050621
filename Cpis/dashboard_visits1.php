...
function svg_alluvial($days, $users, $colors) {
    ...
    for ($d = 0; $d < $nD - 1; $d++) {
        ...
        for ($u = 0; $u < $nU; $u++) {
            ...
            $out .= '<path d="'.$p.'" fill="'.$col.'" fill-opacity="0.8"/>';
        }
    }
    for ($d = 0; $d < $nD; $d++) {
        for ($u = 0; $u < $nU; $u++) {
            ...
            $col = $colors[$u % count($colors)];
            $out .= '<rect x="'.$x[$d].'" y="'.$y0[$d][$u].'" width="'.$nodeW.'" height="'.$hgt.'" rx="3" fill="'.$col.'"/>';
            if ($v > 0 && $hgt >= 18) {
                $out .= '<text x="'.($x[$d]+$nodeW/2).'" y="'.($y0[$d][$u]+$hgt/2+6).'" text-anchor="middle" font-size="16" fill="#fff" font-weight="700">'.$v.'</text>';
            } elseif ($v > 0) {
                $out .= '<text x="'.($x[$d]+$nodeW/2).'" y="'.($y0[$d][$u]+$hgt/2+5).'" text-anchor="middle" font-size="13" fill="#fff" font-weight="700">'.$v.'</text>';
            }
        }
...
$currentYear = sprintf('%04d', $jnow[0]);
$year = isset($_POST['year']) ? preg_replace('/\D/', '', $_POST['year']) : $currentYear;
if (strlen($year) < 4) $year = $currentYear;
...
if (!isset($dbh) || !($dbh instanceof PDO)) {
    ...
    try {
        ...
        if ($codes) {
            ...
            $prefix = $year . '/' . $month . '/%';
            $st = $dbh->prepare("SELECT date, time, PersName, PersCode FROM Last_user WHERE date LIKE ? AND PersCode IN ($place) ORDER BY date DESC, time DESC");
...
$years = array();
for ($yi = (int)$currentYear - 1; $yi <= (int)$currentYear + 1; $yi++) {
    $years[] = sprintf('%04d', $yi);
}
...
// --- KPI direction ---
$kpi_diff = (int)$data['kpi_today'] - (int)$data['kpi_yesterday'];
if ($kpi_diff > 0) { $kpi_dir = 'up'; $kpi_arrow = '▲'; }
elseif ($kpi_diff < 0) { $kpi_dir = 'down'; $kpi_arrow = '▼'; }
else { $kpi_dir = 'same'; $kpi_arrow = '—'; }
$kpi_pct = (int)$data['kpi_yesterday'] > 0 ? round(abs($kpi_diff) * 100 / (int)$data['kpi_yesterday']) : 0;

// --- CSV Export ---
if (isset($_GET['export']) && $_GET['export'] === 'csv') {
    header('Content-Type: text/csv; charset=utf-8');
    header('Content-Disposition: attachment; filename="visits_' . $year . '_' . $month . '.csv"');
    $fp = fopen('php://output', 'w');
    fprintf($fp, chr(0xEF).chr(0xBB).chr(0xBF)); // BOM for Excel
    fputcsv($fp, array('تاریخ', 'ساعت', 'نام کاربری', 'نام و نام خانوادگی'));
    if (!empty($data['active'])) {
        foreach ($logs as $lg) {
            fputcsv($fp, array($lg['date'], $lg['time'], $lg['PersCode'], $lg['PersName'])) ;
        }
    }
    fclose($fp);
    exit;
...
.tleg span { display:flex; align-items:center; gap:8px; font-size:12px; line-height:1.7; color:#1e293b; }
.tleg i { width:9px; height:9px; border-radius:50%; display:inline-block; flex-shrink:0; }
...
.drop-list label { display:flex; gap:8px; align-items:center; padding:7px 8px; font-size:13px; cursor:pointer; border-radius:8px; }
...
/* KPI direction colors */
.kpi.up { background: linear-gradient(135deg, #d5f5e3, #a9dfbf); }
.kpi.up .n { color: #1e8449; }
.kpi.up .kpi-dir { color: #27ae60; }
.kpi.down { background: linear-gradient(135deg, #fadbd8, #f5b7b1); }
.kpi.down .n { color: #922b21; }
.kpi.down .kpi-dir { color: #e74c3c; }
.kpi.same .kpi-dir { color: #7f8c8d; }
.kpi-dir { font-size:14px; font-weight:700; margin-top:4px; }
.kpi-pct { font-size:11px; color:#64748b; margin-top:2px; }
/* Animations */
.card, .kpi { animation: fadeUp .4s ease-out both; }
...
.export-btn {
    ...
    background:linear-gradient(135deg,#2b7bb9,#1a6fa0); color:#fff;
    font-family:inherit; font-size:13px; font-weight:600;
    cursor:pointer; text-decoration:none; transition:all .2s;
}
.export-btn:hover { background:linear-gradient(135deg,#1a6fa0,#155d87); box-shadow:0 4px 12px rgba(43,123,185,.3); transform:translateY(-1px); }
...
.month-badge .n { font-size:32px; font-weight:700; line-height:1; }
.month-badge .l { font-size:11px; opacity:.9; margin-top:6px; }
/* Card shadow on hover */
.card:hover, .kpi:hover { box-shadow:0 6px 24px rgba(0,0,0,.06); }
/* Print styles */
@media print {
    body { background:#fff; }
    .page { padding:0; max-width:100%; }
    .card, .kpi, .month-badge { box-shadow:none; border:1px solid #ddd; break-inside:avoid; }
    .export-btn, .pill, .drop { display:none !important; }
    .layout { grid-template-columns:200px 1fr 1fr !important; }
    h1 { font-size:18px; }
}
...
<div class="page">
    <div class="topbar" style="justify-content:center; position:relative;">
      ...
      <a class="export-btn" href="<?php echo $self; ?>?export=csv&year=<?php echo h($year); ?>&month=<?php echo h($month); ?>" style="position:absolute; left:0;">
    ...
    <div class="layout">
        <aside class="side">
            ...
            <div class="kpi <?php echo $kpi_dir; ?>">
                <div class="n"><?php echo (int)$data['kpi_today']; ?></div>
                <div class="kpi-dir"><?php echo $kpi_arrow; ?></div>
                <div class="kpi-pct"><?php echo $kpi_pct > 0 ? $kpi_pct.'٪ '.($kpi_dir==='up'?'افزایش':'کاهش') : 'بدون تغییر'; ?></div>
                <div class="l"><?php echo h($data['kpi_today_label']); ?></div>
            </div>
            <div class="kpi">
                <div class="n"><?php echo (int)$data['kpi_yesterday']; ?></div>
                <div class="l"><?php echo h($data['kpi_yesterday_label']); ?></div>
            </div>
            <div class="month-badge">
                <div class="n"><?php echo $monthTotal; ?></div>
                <div class="l">مجموع ورودهای ماه</div>
            </div>
            <div class="card drop" id="groupDrop">
                <div class="meta" style="margin-bottom:8px">گروه کاربری</div>
                <button type="button" class="drop-btn" id="groupBtn"><?php echo h($groupBtn); ?></button>
                <div class="drop-list">
                    <?php foreach ($CHIEF_LABELS as $cid => $clab): ?>
                        <label>
                            <input type="checkbox" name="chief[]" value="<?php echo h((string)$cid); ?>" <?php echo in_array((string)$cid, $chiefs, true) ? 'checked' : ''; ?> onchange="this.form.submit()">
                            <?php echo h($clab); ?>
                        </label>
                    <?php endforeach; ?>
                </div>
        ...
        <div class="card trend-card">
            ...
            <?php else: ?>
            <div class="flowbox">
                <div class="flow"><?php echo svg_alluvial($data['trend_days'], $data['trend_users'], $PAL); ?></div>
                <div class="tleg" dir="rtl">
                    <div class="tleg-h">نام کاربر</div>
                    <?php foreach ($data['trend_users'] as $ui => $tu): ?>
                    <span><i style="background:<?php echo $PAL[$ui % 8]; ?>"></i><?php echo h($tu['name']); ?></span>
                    <?php endforeach; ?>
                </div>
            </div>
            <?php endif; ?>
...
(function(){
    ...
    if (!d || !b) return;
    b.onclick = function(e){ e.preventDefault(); d.className = d.className.indexOf('open')>=0 ? 'card drop' : 'card drop open'; };
    document.addEventListener('click', function(e){ if (!d.contains(e.target)) d.className = 'card drop'; });
})();
</script>
</body>
</html>
